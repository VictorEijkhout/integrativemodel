...
TEST_CASE( "OMP distributions, local stuff","[omp][distribution][13]" ) {
  ...
  REQUIRE_NOTHROW( d2 = shared_ptr<distribution>( new omp_distribution(d1) ) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    domain_coordinate cs = domain_coordinate(vector<index_int>{s});
    CHECK( !d1->contains_element(mycoord,cs) );
    domain_coordinate l1 = l+1;
    CHECK( ( !d1->is_valid_index(l+11) || !d1->contains_element(mycoord,l1) ) );
    domain_coordinate m1 = domain_coordinate(vector<index_int>{-1});
    CHECK( !d1->contains_element(mycoord,m1) );
  }
  
  shared_ptr<distribution> d3;
  REQUIRE_NOTHROW( d3 = shared_ptr<distribution>( new omp_replicated_distribution(decomp) ) );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
...
TEST_CASE( "OMP distributions at nonzero offset, local stuff","[omp][distribution][15]" ) {
  ...
  REQUIRE_NOTHROW( pstr.create_from_indexstruct
		   ( shared_ptr<indexstruct>( new contiguous_indexstruct(base,base+s-1) ) ) );

  shared_ptr<distribution> d1;
  REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_distribution(pstr) ) );
  CHECK( d1->has_defined_type() );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
  ...
  index_int d2s,d2f;
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord; shared_ptr<multi_indexstruct> struc;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    auto cl = domain_coordinate(vector<index_int>{l});
    CHECK( d1->contains_element(mycoord,cl) );
    auto bs = domain_coordinate(vector<index_int>{base+s});
    ...
    auto b1 = domain_coordinate(vector<index_int>{base-1});
...
TEST_CASE( "Operated distributions with modulo","[omp][distribution][modulo][24]" ) {
  ...
  int gsize = 10*ntids;
  shared_ptr<distribution> d1 = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,-1,gsize) );
  ...
  shared_ptr<distribution> d1shift;
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    auto cfs = domain_coordinate(vector<index_int>{fshift});
...
TEST_CASE( "Masked distribution creation","[distribution][mask][70]" ) {
  ...
  INFO( "masked created by: " << to_string(path) );

  auto
    block = shared_ptr<distribution>( new omp_block_distribution(decomp,localsize,-1) ),
    masked_block = shared_ptr<distribution>( new omp_distribution( block ) );
  REQUIRE_NOTHROW( masked_block->add_mask(mask) );

  auto
    whole_vector = shared_ptr<object>( new omp_object(block) ),
    masked_vector = shared_ptr<object>( new omp_object(masked_block) );
...
TEST_CASE( "Masked distribution on output","[distribution][mask][71]" ) {
  ...
  shared_ptr<vector<double>> indata,outdata;
  REQUIRE_NOTHROW( indata = whole_vector->get_data(new processor_coordinate_zero(1)) );
  for (index_int i=0; i<whole_vector->global_volume(); i++) indata->at(i) = 1.;
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
	...
	n = masked_vector->volume(mycoord);
      ...
      for (index_int i=f; i<f+n; i++) outdata->at(i) = 2.;
    } else {
  ...
  REQUIRE_NOTHROW
    ( copy = shared_ptr<kernel>( new omp_kernel(whole_vector,masked_vector,fmt::format("copy-to-masked")) ) );
  copy->set_last_dependency()->set_explicit_beta_distribution(masked_vector);
  copy->set_localexecutefn( &veccopy );
  algorithm *queue = new omp_algorithm(decomp);
  REQUIRE_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(whole_vector) ) ) );
  ...
  for (int mytid=0; mytid<ntids; mytid++)
    if (mytid==alive) {
      ...
      CHECK( !halo->lives_on(mytid) );
  vector<task*> *tsks; REQUIRE_NOTHROW( tsks = copy->get_tasks() );
...
TEST_CASE( "Masked distribution on input","[distribution][mask][72]" ) {
  ...
  for (index_int i=0; i<whole_vector->global_volume(); i++) outdata->at(i) = 1.;
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
	...
	n = masked_vector->volume(mycoord);
      for (index_int i=f; i<f+n; i++) indata->at(i) = 2.;
    } else {
      REQUIRE_THROWS( indata = masked_vector->get_data(decomp.coordinate_from_linear(mytid)) );
    }
  }
  shared_ptr<kernel> copy;
  REQUIRE_NOTHROW( copy = shared_ptr<kernel>( new omp_copy_kernel(masked_vector,whole_vector) ) );

  algorithm *queue = new omp_algorithm(decomp);
  REQUIRE_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(masked_vector) ) ) );
  REQUIRE_NOTHROW( queue->add_kernel( copy ) );
  REQUIRE_NOTHROW( queue->analyze_dependencies() );
  dependency *dep;
...
TEST_CASE( "omp-specific mask allocation issues","[distribution][mask][79]" ) {
  ...
  shared_ptr<distribution> block = shared_ptr<distribution>( new omp_block_distribution(decomp,localsize,-1) );
  ...
  object *vector;
  REQUIRE_NOTHROW( vector = shared_ptr<object>( new omp_object(block) ) );
  CHECK( vector->global_allocation()==2*localsize );
  CHECK( !vector->lives_on(1) );
  shared_ptr<vector<double>> dt;
...
TEST_CASE( "dividing","[distribution][ortho][15]" ) {
  ...
  INFO( "k=" << k );
  REQUIRE_NOTHROW( level_dist = shared_ptr<distribution>( new omp_block_distribution(decomp,k,nlocal,-1) ) );
...
TEST_CASE( "extending distributions","[distribution][extend][30]" ) {
  ...
  REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal,-1) ) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    if (mytid!=0) {
      auto left = shared_ptr< multi_indexstruct >
    ...
    if (mytid!=ntids-1) {
      auto right = shared_ptr< multi_indexstruct >
	( new multi_indexstruct( shared_ptr<indexstruct>
				 ( new contiguous_indexstruct
				   ( the_last+shift ) ) ) );
...
TEST_CASE( "Cyclic distributions","[distribution][cyclic][40]" ) {
  ...
  //  REQUIRE_THROWS( d = shared_ptr<distribution>( new omp_cyclic_distribution(decomp,-1,-1) ) ); VLE why is this disabled?
  //  REQUIRE_THROWS( d = shared_ptr<distribution>( new omp_cyclic_distribution(decomp,1,ntids+1) ) );
  SECTION( "set from local" ) {
    REQUIRE_NOTHROW( d = shared_ptr<distribution>( new omp_cyclic_distribution(decomp,2,-1) ) );
  }
  SECTION( "set from global" ) {
    REQUIRE_NOTHROW( d = shared_ptr<distribution>( new omp_cyclic_distribution(decomp,-1,2*ntids) ) );
  }
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
...
TEST_CASE( "Function-specified distribution","[distribution][50]" ) {
  ...
  CHECK_NOTHROW( d1 = shared_ptr<distribution>( new omp_distribution(decomp,&pfunc1,nlocal ) ) );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
    INFO( "mytid=" << mytid );
    ...
    for (int i=0; i<nlocal; i++) {
      ...
      auto cig = domain_coordinate(vector<index_int>{iglobal});
      CHECK( d1->contains_element(mycoord,cig) );
      CHECK( d1->find_index(iglobal)==mytid );
    }
  }

  CHECK_NOTHROW( d2 = shared_ptr<distribution>( new omp_distribution(decomp,&pfunc2,nlocal ) ) );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
...
TEST_CASE( "multidimensional distributions","[multi][distribution][100]" ) {
  ...
  //   REQUIRE_NOTHROW( mdecomp = omp_decomposition(arch,grid) );
  // }
  ...
  REQUIRE_NOTHROW( d = shared_ptr<distribution>
		   ( new omp_block_distribution(mdecomp,domain_layout) ) );
...
TEST_CASE( "multidimensional distributions error test","[multi][distribution][101]" ) {
  ...
  SECTION( "explicit splitting of processor grid" ) {
    ...
    REQUIRE_NOTHROW( mdecomp = omp_decomposition(arch,endpoint) );
  }
  ...
  //   REQUIRE_THROWS( mdecomp = omp_decomposition(arch,endpoint) );
  // }
...
TEST_CASE( "multidimensional data layout","[multi][distribution][data][102]" ) {
  ...
  shared_ptr<distribution> d;
  REQUIRE_NOTHROW
    ( d = shared_ptr<distribution>( new omp_block_distribution(mdecomp,domain_layout) ) );
  CHECK( d->get_dimensionality()==2 );
  CHECK( d->global_volume()==4 );

  shared_ptr<object> o;
  REQUIRE_NOTHROW( o = shared_ptr<object>( new omp_object(d) ) );
  REQUIRE_NOTHROW( o->allocate() );
  for (int i=0; i<2; i++) {
    for (int j=0; j<2; j++) {
      processor_coordinate p;
      REQUIRE_NOTHROW( p = processor_coordinate( vector<int>{i,j} ) );
      //shared_ptr<vector<double>> data;
      decltype(o->get_data(p)) data;
      REQUIRE_NOTHROW( data = o->get_data(p) );
      index_int loc;
      REQUIRE_NOTHROW( loc = d->location_of_first_index(d,p) );
      fmt::print("Proc {} has index at {}\n",p.as_string(),loc);
      REQUIRE_NOTHROW( data.at(loc) = loc );
    }
  }
  auto data = o->get_numa_data_pointer();
  for (int i=0; i<4; i++)
    CHECK( data->at(i)==Approx(i) );
}
...
TEST_CASE( "More masked distribution on output","[distribution][mask][700]" ) {
  ...
  REQUIRE_NOTHROW( block = shared_ptr<distribution>( new omp_block_distribution(decomp,localsize,-1) ) );
  REQUIRE_NOTHROW
    ( masked_block = shared_ptr<distribution>( new omp_distribution( block ) ) );
  REQUIRE_NOTHROW( masked_block->add_mask(mask) );
  ...
  REQUIRE_NOTHROW( whole_vector  = shared_ptr<object>( new omp_object(block) ) );
  REQUIRE_NOTHROW( masked_vector = shared_ptr<object>( new omp_object(masked_block) ) );
  REQUIRE_NOTHROW( whole_vector->allocate() );
  REQUIRE_NOTHROW( masked_vector->allocate() );
  for (int mytid=0; mytid<ntids; mytid++) {
    INFO( " mytid = " << mytid );
  ...
  algorithm *queue = new omp_algorithm(decomp);
  REQUIRE_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(whole_vector) ) ) );
  shared_ptr<kernel> copy = shared_ptr<kernel>( new omp_kernel(whole_vector,masked_vector) );
...
TEST_CASE( "Two masks","[distribution][mask][71]" ) {
  ...
  auto
    block = shared_ptr<distribution>( new omp_block_distribution(decomp,localsize,-1) ),
    masked_block1 = shared_ptr<distribution>( new omp_distribution( block ) ),
    masked_block2 = shared_ptr<distribution>( new omp_distribution( block ) );
  REQUIRE_NOTHROW( masked_block1->add_mask(mask1) );
  REQUIRE_NOTHROW( masked_block2->add_mask(mask2) );
  auto
    whole_vector = shared_ptr<object>( new omp_object(block) ),
    masked_vector1 = shared_ptr<object>( new omp_object(masked_block1) ),
    masked_vector2 = shared_ptr<object>( new omp_object(masked_block2) );
  auto
    copy1 = shared_ptr<kernel>( new omp_kernel(whole_vector,masked_vector1) ),
    copy2 = shared_ptr<kernel>( new omp_kernel(masked_vector1,masked_vector2) );
