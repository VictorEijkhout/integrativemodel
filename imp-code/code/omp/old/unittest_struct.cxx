...
TEST_CASE( "checking on index offset calculation","[index][07]" ) {
  ...
  decomposition decomp; REQUIRE_NOTHROW( decomp  = omp_decomposition(arch,layout) );
  ...
  domain_coordinate farpoint( domain );
  shared_ptr<distribution> d; shared_ptr<object> v;
  REQUIRE_NOTHROW
    ( d = shared_ptr<distribution>( new omp_block_distribution(decomp,farpoint) ) );
  CHECK( d->get_dimensionality()==dim );
  REQUIRE_NOTHROW( v = shared_ptr<object>( new omp_object(d) ) );
  REQUIRE_NOTHROW( v->set_name("v07") );

  shared_ptr<kernel> make;
  REQUIRE_NOTHROW( make = shared_ptr<kernel>( new omp_origin_kernel(v) ) );
  REQUIRE_NOTHROW( make->set_localexecutefn(&vecsetconstantp) );
  algorithm maker = omp_algorithm(decomp);
  REQUIRE_NOTHROW( maker.add_kernel(make) );
  REQUIRE_NOTHROW( maker.analyze_dependencies() );
  REQUIRE_NOTHROW( maker.execute() );

  shared_ptr<vector<double>> data; index_int dsize;
  REQUIRE_NOTHROW( data = v->get_numa_data_pointer() );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    shared_ptr<multi_indexstruct> pstruct;
    decltype(v->get_numa_structure()) gstruct,nstruct;
    ...
    CHECK( data->at(offset)==Approx( (double)mytid ) );
  }
}

TEST_CASE( "Analyze single interval","[index][structure][12]" ) {
  int localsize = 5;
  auto dist = shared_ptr<distribution>( new omp_distribution(decomp) ); 
  CHECK_NOTHROW( dist->create_from_global_size(localsize*ntids) );
  vector<shared_ptr<message>> mm;
  shared_ptr<message> m; shared_ptr<multi_indexstruct> s; shared_ptr<multi_indexstruct> segment;
  ...
  SECTION( "find an exact processor interval" ) {
    ...
    for (int mytid=0; mytid<ntids; mytid++) {
      processor_coordinate mycoord;
      REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
      segment = shared_ptr<multi_indexstruct>
	( new multi_indexstruct
	  ( shared_ptr<indexstruct>( new contiguous_indexstruct(0,localsize-1) ) ) );
      CHECK_NOTHROW( mm = dist->messages_for_segment
		     ( 0,mycoord,self_treatment::INCLUDE,segment,segment ) );
      CHECK( mm.size()==1 );
      m = mm.at(0);
      CHECK( m->get_sender()== zero  );
  ...
  SECTION( "find a sub interval" ) {
    for (int mytid=0; mytid<ntids; mytid++) {
      ...
      segment = shared_ptr<multi_indexstruct>
	( new multi_indexstruct
	  ( shared_ptr<indexstruct>( new contiguous_indexstruct(1,localsize-2) ) ));
	CHECK_NOTHROW( mm = dist->messages_for_segment
		       ( 0,mycoord,self_treatment::INCLUDE,segment,segment ) );
      CHECK( mm.size()==1 );
      m = mm.at(0);
      CHECK( m->get_sender()== processor_coordinate_zero(1)  );
      CHECK( m->get_receiver()==mycoord );
  ...
  SECTION( "something with nontrivial intersection" ) {
	...
	segment = shared_ptr<multi_indexstruct>
	  ( new multi_indexstruct
	    ( shared_ptr<indexstruct>( new contiguous_indexstruct(0,localsize) ) ));
	CHECK_NOTHROW( mm = dist->messages_for_segment
		       ( 0,mycoord,self_treatment::INCLUDE,segment,segment ) );
	CHECK( mm.size()==2 );
	for (int im=0; im<2; im++) { // we don't insist on ordering
	  ...
	  if (m->get_sender()==processor_coordinate_zero(1) ) {
...
TEST_CASE( "OMP distributions","[omp][distribution][replicated][13]" ) {
  ...
  SECTION( "block" ) {
    REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_block_distribution(decomp,s) ) );
    ...
    for (int mytid=0; mytid<ntids; mytid++) {
      ...
      auto cf = domain_coordinate(vector<index_int>{f});
      CHECK( d1->contains_element(mycoord,cf) );
      auto f1 = domain_coordinate(vector<index_int>{f-1});
      CHECK( ( !d1->is_valid_index(f1) || !d1->contains_element(mycoord,f1) ) );
    
      CHECK( d1->last_index_r(mycoord)[0]==l );
      auto lm1 = domain_coordinate(vector<index_int>{l-1});
      CHECK( d1->contains_element(mycoord,lm1) );
      auto ds = domain_coordinate(vector<index_int>{s});
      CHECK( !d1->contains_element(mycoord,ds) );
      auto l1 = domain_coordinate(vector<index_int>{l+1});
      CHECK( ( !d1->is_valid_index(l1) || !d1->contains_element(mycoord,l1) ) );
      auto m1 = domain_coordinate(vector<index_int>{-1});
      CHECK( !d1->contains_element(mycoord,m1) );
    }
  }

  SECTION( "replicated" ) {
    REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_replicated_distribution(decomp) ) );
    REQUIRE_NOTHROW( d1->get_domains() );
    CHECK( d1->get_domains().size()==ntids );
    // for (int mytid=0; mytid<ntids; mytid++) {
    //   processor_coordinate mycoord;
      // REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
    for ( auto mycoord : d1->get_domains() ) {
    ...
    CHECK( d1->get_domains().size()==ntids );
    REQUIRE_NOTHROW( scalar = shared_ptr<object>( new omp_object(d1) ) );
    REQUIRE_NOTHROW( scalar->allocate() );
    data_pointer d,d0;
    REQUIRE_NOTHROW( d = scalar->get_data(processor_coordinate_zero(1)) );
    d0 = d;
    for (int i=0; i<ntids; i++) {
      double v = i+1;
      print("set data at {} to {}\n", (long)(d.data()+i),v);
      REQUIRE_NOTHROW( d.data()[i] = v );
    }
    INFO( "replicated storage starts at " << (long)d0.data() );
    auto dptr = scalar->get_data(processor_coordinate_zero(1));
    CHECK( dptr.size()==1 );
    for (index_int i=0; i<ntids; i++) {
      INFO( "proc " << i );
      processor_coordinate mycoord;
      REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(i) );
      dptr = scalar->get_data(mycoord);
      double v;
      REQUIRE_NOTHROW( v = dptr.at(0) );
      print("[{}={}] .. read at {}: {}\n",
	    i,mycoord.as_string(),(long)dptr.data(),v);
      CHECK( v==Approx(i+1) );
    }
  }
  ...
  SECTION( "shifted" ) {
    ...
    auto domain = shared_ptr<indexstruct>( new contiguous_indexstruct(bottom,top) );
    parallel_structure parallel(decomp);
    REQUIRE_NOTHROW( parallel.create_from_indexstruct(domain) );
    REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_distribution(/*decomp,*/parallel) ) );
...
TEST_CASE( "Analyze one dependency","[operate][dependence][modulo][16]") {
  ...
  auto alpha = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,localsize*ntids) );
  ioperator shiftop;
  shared_ptr<multi_indexstruct> alpha_block, segment,halo;
  //  vector<indexstruct*> *beta_blocks;
  vector<shared_ptr<message>> mm; shared_ptr<message> m;
  ...
  SECTION( "right modulo" ) {
	...
	if (m->get_sender().coord(0)==mytid) {
	  ...
	  if (mytid==ntids-1) CHECK( m->get_sender()==processor_coordinate_zero(1)  );
	  CHECK( m->get_global_struct()->local_size(0)==1 );
...
TEST_CASE( "Analyze one dependency, shifted domain","[domain][operate][dependence][016]") {
  ...
  REQUIRE_NOTHROW( alpha = shared_ptr<distribution>( new omp_distribution(/*decomp,*/parallel) ) );
  
  ioperator shiftop;
  shared_ptr<multi_indexstruct> alpha_block;
  //  vector<indexstruct*> *beta_blocks;
  vector<shared_ptr<message>> mm; shared_ptr<message> m;
  index_int my_first_index,my_last_index;
  shared_ptr<multi_indexstruct> segment,halo;
...
TEST_CASE( "Analyze processor dependencies, right modulo, include self message",
  ...
  auto d1 = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,10*ntids) );
  shared_ptr<multi_indexstruct> h,beta_block;
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    CHECK_NOTHROW( mm = d1->messages_for_segment
		   ( 0,mycoord,self_treatment::INCLUDE,beta_block,h) );
    //    CHECK_NOTHROW( mm = d1->analyze_one_dependence(mytid,0,no_op,d1,h) );
    CHECK( mm.size()==1 );
    m0 = mm.at(0);
    ...
    CHECK_NOTHROW( mm = d1->messages_for_segment
		   ( 0,mycoord,self_treatment::INCLUDE,beta_block,h) );
    //CHECK_NOTHROW( mm = d1->analyze_one_dependence(mytid,0,right_shift,d1,h) );
...
TEST_CASE( "Analyze processor dependencies, right modulo, skip self",
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    beta_block = d1->get_processor_structure(mycoord)->operate(no_op);
    CHECK_NOTHROW( mm = d1->messages_for_segment
		   ( 0,mycoord,self_treatment::EXCLUDE,beta_block,h) );
    CHECK( mm.size()==0 ); // skip self: nothing remains
    beta_block = d1->get_processor_structure(mycoord)->operate(no_op);
    CHECK_NOTHROW( mm = d1->messages_for_segment
		   ( 0,mycoord,self_treatment::INCLUDE,beta_block,h) );
    CHECK( mm.size()==1 ); // this is the real case
    m0 = mm.at(0);
    auto global_struct = m0->get_global_struct(),
      local_struct = m0->get_local_struct();
    index_int f = global_struct->first_index_r()[0],l = global_struct->last_index_r()[0];
    ...
    int s0;
    shared_ptr<multi_indexstruct> src0,tar0;
    ...
    h = shared_ptr<multi_indexstruct>
      ( new multi_indexstruct
	( shared_ptr<indexstruct>( new contiguous_indexstruct(myfirst-1,mylast) ) ));
...
TEST_CASE( "Analyze processor dependencies, left bump","[index][message][19]") {
  ...
  shared_ptr<distribution> d1;
  REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_block_distribution(decomp,10*ntids) ) );
  shared_ptr<multi_indexstruct> h,beta_block;
...
TEST_CASE( "Task dependencies threepoint bump","[task][message][object][21]" ) {
  ...
  shared_ptr<distribution> d1;
  REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( new omp_block_distribution(decomp,g) ) );

  shared_ptr<object> o1,r1;
  CHECK_NOTHROW( r1 = shared_ptr<object>( new omp_object(d1) ) );
  CHECK_NOTHROW( o1 = shared_ptr<object>( new omp_object(d1) ) );
  ...
  CHECK_NOTHROW( beta_dist = shared_ptr<distribution>( new omp_distribution(preds) ) );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
...
TEST_CASE( "Task dependencies threepoint modulo","[task][message][object][modulo][22]" ) {
  ...
  auto d1 = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal*ntids) );
  shared_ptr<object> o1,r1;
  CHECK_NOTHROW( r1 = shared_ptr<object>( new omp_object(d1) ) );
  CHECK_NOTHROW( o1 = shared_ptr<object>( new omp_object(d1) ) );
  ...
  shared_ptr<distribution> beta_dist;
  CHECK_NOTHROW( beta_dist = shared_ptr<distribution>( new omp_distribution(/*decomp,*/preds) ) );
  ...
  shared_ptr<kernel> combine;
  shared_ptr<task> combine_task;
  REQUIRE_NOTHROW( combine = shared_ptr<kernel>( new omp_kernel(o1,r1) ) );
  REQUIRE_NOTHROW( combine->split_to_tasks());
  REQUIRE_NOTHROW( combine->set_explicit_beta_distribution( 0,beta_dist ) );
  REQUIRE_NOTHROW( combine->set_last_dependency().create_beta_vector(beta_dist) );
...
TEST_CASE( "Task send structure threepoint bump","[task][message][object][23]" ) {
  ...
  auto d1 = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,gsize) );
  ...
  multi_indexstruct enclosing;
    ...
    kernel originate = omp_kernel(o1,r1);
    shared_ptr<task> originate_task;
    CHECK_NOTHROW( originate.set_explicit_beta_distribution( beta_dist ) );
    // VLE is this r1 correct?
    shared_ptr<distribution> halo_dist;
    REQUIRE_NOTHROW( halo_dist = originate.last_dependency().find_beta_distribution(r1) );
    ...
    for (int mytid=0; mytid<ntids; mytid++) {
      processor_coordinate mycoord;
      REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
      auto my_first = d1->first_index_r(mycoord),my_last = d1->last_index_r(mycoord);
  ...
  multi_indexstruct enclosing;
	...
	auto csf = domain_coordinate(vector<index_int>{sf});
	CHECK( d1->contains_element(m->get_sender(),csf) );
	auto csl = domain_coordinate(vector<index_int>{sl});
...
TEST_CASE( "Task execute on local data","[object][task][29]" ) {
  ...
  index_int s = 10;
  auto block = shared_ptr<distribution>( new omp_block_distribution(decomp,s*ntids) );
  for (int mytid=0; mytid<ntids; mytid++) {
    processor_coordinate mycoord;
    REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
    CHECK( block->volume(mycoord)==s );
  }
  auto vec = shared_ptr<object>( new omp_object(block) );
  REQUIRE_NOTHROW( vec->allocate() );
  REQUIRE( vec->has_data_status_allocated() );

  kernel k = omp_kernel(vec);
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    CHECK_NOTHROW( t->local_execute(vec) );
    data_pointer data;
    REQUIRE_NOTHROW( data = vec->get_data(mycoord) );
...
TEST_CASE( "Task execute on external data","[object][task][30]" ) {
  ...
  index_int s = 13;
  auto xdata = make_shared<vector<double>>(ntids*s);
  //fmt::print("data allocated at {}\n",(long int)xdata);
  auto block = shared_ptr<distribution>( new omp_block_distribution(decomp,s*ntids) );
  auto xvector = shared_ptr<object>( new omp_object(block,xdata) );

  kernel xk = omp_kernel(xvector);
  xk.set_localexecutefn(&vector_gen);
  REQUIRE_NOTHROW( xk.analyze_dependencies() ); //split_to_tasks());
  REQUIRE_NOTHROW( xk.execute() );
...
TEST_CASE( "Execute local to local task","[task][halo][execute][31]" ) {
  ...
  auto block = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal*ntids) );
  auto xdata = make_shared<vector<double>>(nlocal);
  auto 
  ...
  shared_ptr<task> copy_task,scale_task;
  data_pointer ydata;

  SECTION( "copy kernel" ) {
    shared_ptr<kernel> copy = shared_ptr<kernel>( new omp_kernel(xvector,yvector) );
	...
	mylast = block->last_index_r(mycoord);
      ...
      // CHECK_NOTHROW( ydata = yvector->get_data(processor_coordinate_zero(1)) );
      // for (int i=0; i<nlocal; i++)
      // 	CHECK( ydata->at(i) == Approx( (double)(1.5+i) ) );
    }
  }

  SECTION( "scale kernel" ) {
    //  let's try the same with scaling
    shared_ptr<kernel> scale = shared_ptr<kernel>( new omp_kernel(xvector,yvector) );
    scale->add_sigma_operator( no_op );
    scale->set_localexecutefn(&vecscalebytwo);

    REQUIRE_NOTHROW( scale->analyze_dependencies() );
    //CHECK_NOTHROW( scale->set_last_dependency().ensure_beta_distribution(yvector) );
...
TEST_CASE( "Execute shift task","[task][halo][execute][32]" ) {
  ...
  auto xdata = make_shared<vector<double>>(gsize);
  auto
    xvector = shared_ptr<object>( new omp_object(block,xdata) ),
    yvector = shared_ptr<object>( new omp_object(block) );
  for (int i=0; i<gsize; i++)
    xdata->at(i) = pointfunc33(i,0);
  omp_kernel shift(xvector,yvector);
  ...
  //CHECK_NOTHROW( shift.set_last_dependency().ensure_beta_distribution(yvector) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    shared_ptr<task> shift_task = tasks.at(mytid);
    ...
    for (int imsg=0; imsg<nmsgs; imsg++) {
      shared_ptr<message> msg = msgs[imsg];
  ...
  //   	  CHECK( ydata->at(i+my_first) == Approx( pointfunc33(i+1,my_first) ) );
  //   	}
  //     } else {
  //   	for (i=0; i<nlocal; i++) {
  //   	  INFO( "i=" << i << " yi=" << ydata->at(i) );
...
TEST_CASE( "Scale kernel","[task][kernel][execute][33]" ) {
  ...
  //   CHECK_NOTHROW( scale_task->get_beta_object(0)->get_data(processor_coordinate_zero(1)) ); // we have a halo
  //   CHECK_NOTHROW( ydata = yvector->get_data(processor_coordinate_zero(1)) );
  //   for (int i=0; i<nlocal; i++) {
  //     CHECK( ydata->at(i+my_first) == Approx( 2*pointfunc33(i,my_first) ) );
  //   }
  // }
}
...
TEST_CASE( "Shift kernel modulo","[task][kernel][modulo][execute][34]" ) {
  ...
  auto block = 
    shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal*ntids) );
  auto xdata = make_shared<vector<double>>(gsize);
  //  auto xdata = double[gsize];
  auto
    xvector = shared_ptr<object>( new omp_object(block,xdata) ),
    yvector = shared_ptr<object>( new omp_object(block) );
  for (int i=0; i<gsize; i++)
    xdata->at(i) = pointfunc33(i,0);
  auto shift = omp_kernel(xvector,yvector);
  shift.add_sigma_operator( ioperator("none") );
  shift.add_sigma_operator( ioperator(">>1") );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    // {
    //   int i;
    //   //INFO( "i:" << i <<", iglobal=" << i+my_first << " yi=" << ydata->at(i+my_first) );
    //   if (mytid==ntids-1) {
    //     for (i=0; i<nlocal-1; i++) {
    ...
    //     i = nlocal-1;
    //     REQUIRE( (i+my_first)<gsize );
    //     CHECK( ydata->at(i+my_first) == Approx( pointfunc33(0,0) ) );
    //   } else {
    //     for (i=0; i<nlocal; i++) {
    //       REQUIRE( (i+my_first)<gsize );
    //       CHECK( ydata->at(i+my_first) == Approx( pointfunc33(i+1,my_first) ) );
...
TEST_CASE( "Shift from left kernel, message structure","[task][kernel][35]" ) {
  ...
  for (int i=0; i<gsize; i++)
    xdata->at(i) = pointfunc33(i,0);
  kernel shift = omp_kernel(xvector,yvector);
  shift.add_sigma_operator( ioperator("none") );
  shift.add_sigma_operator( ioperator("<=1") );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    if (mytid>0) { // everyone but the first receives from the left
      CHECK( rmsgs.size()==2 );
      for (int i=0; i<2; i++) {
	shared_ptr<message> msg = rmsgs[i];
...
TEST_CASE( "Shift from left kernel modulo, execute","[task][kernel][execute][modulo][36]" ) {
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    //   if (mytid==0) {
    // 	i = 0;
    // 	CHECK( ydata->at(i+my_first) == Approx(pointfunc33(MOD(i-1,gsize),my_first)) );
    // 	for (i=1; i<nlocal; i++) {
    // 	  INFO( "i=" << i << " yi=" << ydata->at(i+my_first) );
    // 	  CHECK( ydata->at(i+my_first) == Approx(pointfunc33(i-1,my_first)) );
    // 	}
    //   } else {
    // 	for (i=0; i<nlocal; i++) {
    // 	  INFO( "i=" << i << " yi=" << ydata->at(i+my_first) );
    // 	  CHECK( ydata->at(i+my_first) == Approx(pointfunc33(i-1+my_first,0)) );
    // 	}
    //   }
    // }
  }
...
TEST_CASE( "Test explicit beta bump","[beta][distribution][40]" ) {
  ...
  CHECK( ( iscont || isblok ) );
  //  shared_ptr<multi_indexstruct> bigblock;
  multi_indexstruct bigblock;
  ...
  auto
    in = shared_ptr<object>( new omp_object(block) ),
    out = shared_ptr<object>( new omp_object(block) );
  in->set_name("in40"); out->set_name("out40");
  REQUIRE_NOTHROW( in->allocate() );
  REQUIRE( in->has_data_status_allocated() );
  data_pointer indata;
  REQUIRE_NOTHROW( indata = in->get_numa_data_pointer() );
  for (index_int i=0; i<gsize; i++) indata->at(i) = 2.;

  omp_algorithm summing;
  REQUIRE_NOTHROW( summing = omp_algorithm(decomp) );
  REQUIRE_NOTHROW( summing.add_kernel( shared_ptr<kernel>( new omp_origin_kernel(in) ) ) );

  shared_ptr<kernel> threepoint;
  REQUIRE_NOTHROW( threepoint = shared_ptr<kernel>( new omp_kernel(in,out) ) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    // check halo
    shared_ptr<object> halo; data_pointer halodata;
    ...
    for (index_int i=hf; i<=hl; i++) {
      INFO( "i=" << i << " data->at(i)=" << halodata->at(i) );
      CHECK( halodata->at(i) == Approx(2.) );
    }
    // check output
    CHECK( out->volume(mycoord)==localsize );
    data_pointer outdata = out->get_data(mycoord);
    index_int
      f = out->first_index_r(mycoord)[0],
      l = out->last_index_r(mycoord)[0];
    // for (index_int i=f; i<=l; i++) {
    //   INFO( "i=" << i << " data->at(i)=" << outdata->at(i) );
...
TEST_CASE( "Test explicit beta mod","[beta][distribution][modulo][41]" ) {
  ...
  REQUIRE_NOTHROW( in->allocate() );
  REQUIRE( in->has_data_status_allocated() );
  auto indata = in->get_numa_data_pointer();
  for (index_int i=0; i<gsize; i++) indata->at(i) = 2.;

  omp_algorithm summing = omp_algorithm(decomp);
  REQUIRE_NOTHROW( summing.add_kernel( shared_ptr<kernel>( new omp_origin_kernel(in) ) ) );
...
TEST_CASE( "special matrices","[kernel][spmvp][46]" ) {
  ...
  SECTION( "lower diagonal" ) {
	...
	y = shared_ptr<object>( new omp_object(blocked) );
      shared_ptr<kernel> k;
      REQUIRE_NOTHROW( k = shared_ptr<kernel>( new omp_spmvp_kernel( x,y,A ) ) );
	...
	objs[iobj] = shared_ptr<object>( new omp_object(blocked) );
      ...
      data_pointer data;
      REQUIRE_NOTHROW( data = objs[0]->get_numa_data_pointer() ); //decomp.coord(0)) );
	...
	REQUIRE_NOTHROW( data = objs[istep]->get_numa_data_pointer() );//decomp.coord(0)) );
  ...
  SECTION( "upper diagonal" ) {
	...
	objs[iobj] = shared_ptr<object>( new omp_object(blocked) );
      REQUIRE_NOTHROW( objs[0]->allocate() );
      data_pointer data; REQUIRE_NOTHROW( data = objs[0]->get_numa_data_pointer() );
      for (index_int i=0; i<nlocal; i++) data->at(i) = 0.;
	...
	REQUIRE_NOTHROW( queue->add_kernel(k) );
      ...
      for (int istep=0; istep<g; istep++) {
	INFO( "object " << istep << ": " << objs[istep]->values_as_string(mytid) );
	REQUIRE_NOTHROW( data = objs[istep]->get_numa_data_pointer() );
	for (index_int i=my_first; i<=my_last; i++) {
	  INFO( "index " << i );
  ...
  SECTION( "toeplitz" ) {
	...
	REQUIRE_NOTHROW( queue->add_kernel(k) );
      ...
      REQUIRE_NOTHROW( data = objs[nsteps-1]->get_numa_data_pointer() );
      for (index_int i=my_first; i<=my_last; i++) {
	INFO( "index " << i );
...
TEST_CASE( "Kernel data graph: test acyclicity","[kernel][50]" ) {
  ...
  SECTION( "unnamed objects" ) {
    object1 = shared_ptr<object>( new omp_object(block) );
    object2 = shared_ptr<object>( new omp_object(block) );
    object3 = shared_ptr<object>( new omp_object(block) );
    object4 = shared_ptr<object>( new omp_object(block) );

    kernel1 = shared_ptr<kernel>( new omp_kernel(object1,object2) );
    kernel1->set_name("50kernel1");
    kernel2 = shared_ptr<kernel>( new omp_kernel(object1,object3) );
    kernel2->set_name("50kernel2");
    kernel3 = shared_ptr<kernel>( new omp_kernel(object2,object3) );
  ...
  SECTION( "kernel predecessors" ) {
    ...
    kernel1 = shared_ptr<kernel>( new omp_kernel(object1,object2) );
    kernel1->set_last_dependency().set_type_local();
    kernel1->set_name("make2");
    kernel2 = shared_ptr<kernel>( new omp_kernel(object2,object3) );
    kernel2->set_name("make3");
    kernel2->set_last_dependency().set_type_local();
    ...
    // for (vector<shared_ptr<task>>::iterator t=ktasks->begin(); t!=ktasks->end(); ++t) {
    //   vector<int> *preds;
...
TEST_CASE( "Mapping between redundant distros","[redundant][70]" ) {
  ...
  auto
    din = shared_ptr<distribution>( new omp_replicated_distribution(decomp) ),
    dout = shared_ptr<distribution>( new omp_replicated_distribution(decomp) );
  auto
    scalar_in = shared_ptr<object>( new omp_object( din ) ),
    scalar_out = shared_ptr<object>( new omp_object( dout ) );
  auto
    indata = scalar_in->get_numa_data_pointer(),
    outdata = scalar_out->get_numa_data_pointer();
  *indata = 15.3;

  auto copy_kernel = shared_ptr<kernel>( new omp_kernel( scalar_in,scalar_out ) );
  ...
  for ( auto copy_task : tasks ) {
    ...
    CHECK_NOTHROW( copy_task->analyze_dependencies() );
    CHECK( copy_task->get_receive_messages().size()==1 );
    CHECK( copy_task->get_receive_messages()[0]->get_receiver()==mycoord );
    CHECK( copy_task->get_receive_messages()[0]->get_sender()==mycoord );
  }
}
...
TEST_CASE( "Interpolation by restriction", "[stretch][distribution][71][hide]" ) {
  index_int nlocal=10;
  auto
    target_dist = shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal*ntids) ),
    source_dist = shared_ptr<distribution>( new omp_block_distribution(decomp,2*nlocal*ntids) );
  auto
    target = shared_ptr<object>( new omp_object(target_dist) ),
    source = shared_ptr<object>( new omp_object(source_dist) );
  auto restrict = shared_ptr<kernel>( new omp_kernel(source,target) );
  ...
  for ( auto restrict_task : tasks ) {
    ...
    auto msgs = 
      source_dist->messages_for_segment
      ( 0,mycoord,self_treatment::INCLUDE,beta_block,beta_block );
    CHECK( msgs.size()==1 );
    auto mtmp = msgs.at(0);
...
TEST_CASE( "Queue relationships","[queue][100]" ) {
  ...
  REQUIRE_NOTHROW( block = shared_ptr<distribution>( new omp_block_distribution(decomp,nglobal) ) );
  omp_algorithm *queue;
  CHECK_NOTHROW( queue = new omp_algorithm(decomp) );
  auto
    o1 = shared_ptr<object>( new omp_object(block) ),
    o2 = shared_ptr<object>( new omp_object(block) ),
    o3 = shared_ptr<object>( new omp_object(block) );

  SECTION( "short queue relationships" ) {
    shared_ptr<kernel> origin,process,final;
    REQUIRE_NOTHROW( origin = shared_ptr<kernel>( new omp_kernel(o1) ) );
    REQUIRE_NOTHROW( process = shared_ptr<kernel>( new omp_kernel(o1,o2) ) );
    CHECK_NOTHROW( process->set_last_dependency().set_type_local() );
    REQUIRE_NOTHROW( final = shared_ptr<kernel>( new omp_kernel(o2,o3) ) );
    CHECK_NOTHROW( final->set_last_dependency().set_type_local() );
...
TEST_CASE( "Scale queue","[queue][execute][105]" ) {
  ...
  shared_ptr<object> xvector;
  auto yvector = vector<shared_ptr<object>>(nsteps);
  REQUIRE_NOTHROW( xvector = shared_ptr<object>( new omp_object(block,xdata) ) );
  REQUIRE_NOTHROW( xvector->set_name("xvector") );
  CHECK( xvector->has_data_status_allocated() );
  for (int i=0; i<nglobal; i++)
    xdata->at(i) = pointfunc33(i,0/*my_first*/);
  for (int iv=0; iv<nsteps; iv++) {
    INFO( iv );
    REQUIRE_NOTHROW( yvector[iv] = shared_ptr<object>( new omp_object(block) ) );
    REQUIRE_NOTHROW( yvector[iv]->set_name(fmt::format("vector{}",iv)) );
    REQUIRE_NOTHROW( yvector[iv]->allocate() ); printf("lose this allocate!\n");
  }

  shared_ptr<kernel> k = shared_ptr<kernel>( new omp_kernel(xvector) );
  k->set_name( "generate" );
  k->set_localexecutefn(&vecnoset);
  CHECK_NOTHROW( queue->add_kernel(k) );

  for (int iv=0; iv<nsteps; iv++) {
    shared_ptr<kernel> k; char name[20];
    INFO( "step: " << iv );
    if (iv==0) {
      REQUIRE_NOTHROW( k = shared_ptr<kernel>( new omp_kernel(xvector,yvector[0]) ) );
    } else {
      REQUIRE_NOTHROW( k = shared_ptr<kernel>( new omp_kernel(yvector[iv-1],yvector[iv]) ) );
    }
  ...
  for ( auto tsk : tsks ) {
    vector<shared_ptr<message>> rmsgs,smsgs;
  ...
  {
    data_pointer ydata;
    CHECK_NOTHROW( ydata = yvector[nsteps-1]->get_numa_data_pointer() );
    CHECK( ydata!=nullptr );
    for (int i=0; i<nglobal; i++) {
      INFO( "yvalue[" << i << "] = " << ydata->at(i) );
      CHECK( ydata->at(i) == Approx( pow(2,nsteps)*pointfunc33(i,0/*my_first*/) ) );
    }
  }
}
...
TEST_CASE( "Threepoint queue bump","[queue][execute][halo][106]" ) {
  ...
  for (int i=0; i<nglobal; i++)
    xdata->at(i) = 1.;
  for (int iv=0; iv<nsteps; iv++) {
    yvector[iv] = shared_ptr<object>( new omp_object(block) );
  }
  ...
  for (int iv=0; iv<nsteps; iv++) {
    shared_ptr<kernel> k;
    if (iv==0) {
      k = shared_ptr<kernel>( new omp_kernel(xvector,yvector[0]) );
    } else {
      k = shared_ptr<kernel>( new omp_kernel(yvector[iv-1],yvector[iv]) );
    }
    k->add_sigma_operator( no_op );
  ...
  { // do all kernels have a proper step number?
    ...
    for (vector<kernel*>::iterator k=kernels->begin(); k!=kernels->end(); ++k) {
      int kn,kv;
      {
  ...
  for (auto t : queue->get_tasks() ) {
    if (t->get_step()==yvector[0]->get_object_number()) {
      ...
      REQUIRE_NOTHROW( data = h->get_numa_data_pointer() );
	...
	if (mytid==0) {
	  ...
	  CHECK( f==start_own );
	  CHECK( l==last_own+1 );
	  CHECK( data->at(f)==Approx(1.) );
	  CHECK( data->at(l)==Approx(1.) );
  ...
  // the edges are increasingly not a threefold sum.
  {
    for (int s=0; s<nsteps; s++) {
      int i;
      CHECK_NOTHROW( ydata = yvector[s]->get_numa_data_pointer() );
...
TEST_CASE( "Threepoint queue modulo","[queue][execute][halo][modulo][107]" ) {
  ...
  for (auto t=queue->get_tasks().begin(); t!=queue->get_tasks().end(); ++t) {
    if ((*t)->get_step()==yvector[0]->get_object_number()) {
      found++;
      shared_ptr<object> h; data_pointer data; int f0;
      REQUIRE_NOTHROW( h=(*t)->get_beta_object(0) );
      CHECK( h->outer_size()==(nglobal+2) );
      REQUIRE_NOTHROW( data = h->get_numa_data_pointer() );
      //for (int i=0; i<nglobal+2; i++) printf("%e ",data->at(i)); printf("\n");
      for (int mytid=0; mytid<ntids; mytid++) {
	processor_coordinate mycoord;
	REQUIRE_NOTHROW( mycoord = decomp.coordinate_from_linear(mytid) );
...
TEST_CASE( "Scale queue analysis with embedding","[queue][embed][execute][110]" ) {
  ...
  //REQUIRE_NOTHROW( dd = omp_decomposition( dynamic_cast<omp_architecture*>(aa) ) );
  REQUIRE_NOTHROW( dd = omp_decomposition(aa) );
  int nlocal=22;
  ioperator no_op("none");
  auto block = 
    shared_ptr<distribution>( new omp_block_distribution(dd,nlocal*ntids) );
  ...
  auto
    xvector = shared_ptr<object>( new omp_object(block) ),
    yvector = shared_ptr<object>( new omp_object(block) );
  if ( power==0 ) {
  ...
  shared_ptr<kernel> gen_kernel,mult_kernel;
  REQUIRE_NOTHROW( gen_kernel = shared_ptr<kernel>( new omp_origin_kernel(xvector) ) );
  REQUIRE_NOTHROW( mult_kernel = shared_ptr<kernel>( new omp_kernel(xvector,yvector) ) );
...
TEST_CASE( "test embedding by memory counting","[kernel][halo][embed][120]" ) {
  ...
  int do_embed = 0;
  shared_ptr<distribution> block;
  shared_ptr<object> xvector,yvector; algorithm *queue;
  SECTION( "allocate at once" ) { do_embed = 0;
    CHECK( !arch.get_can_embed_in_beta() );
    block = shared_ptr<distribution>( new omp_block_distribution(decomp,nlocal,-1) );
    queue = new omp_algorithm(decomp);
  }
  SECTION( "allocate later" ) { do_embed = 1;
    architecture aa(arch);
    REQUIRE_NOTHROW( omp_architecture(aa) )
    //omp_architecture *aa; REQUIRE_NOTHROW( aa = new omp_architecture(*arch) );
    REQUIRE_NOTHROW( aa.set_can_embed_in_beta() );
    CHECK( aa.get_can_embed_in_beta() );
    omp_decomposition dd; REQUIRE_NOTHROW( dd = omp_decomposition(aa) );
    block = shared_ptr<distribution>( new omp_block_distribution(dd,nlocal,-1) );
  ...
  REQUIRE_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_copy_kernel(xvector,yvector) ) ) );
  REQUIRE_NOTHROW( queue->analyze_dependencies() );
  if (do_embed)
...
TEST_CASE( "test local executability for scaling","[queue][sync][150]" ) {
  ...
  // make origin kernel
  CHECK_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(xvector) ) ) );
...
TEST_CASE( "test local executability; threepoint with sync corners","[queue][sync][151]" ) {
  ...
  for (int i=0; i<nglobal; i++)
    xdata->at(i) = pointfunc33(i,0/*my_first*/);
  REQUIRE_NOTHROW( yvector = shared_ptr<object>( new omp_object(block) ) );

  CHECK_NOTHROW( queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(xvector) ) ) );
  shared_ptr<kernel> k;
  REQUIRE_NOTHROW( k = shared_ptr<kernel>( new omp_kernel(xvector,yvector) ) );
  k->add_sigma_operator( no_op );
  ...
  vector<kernel*> *kerns; REQUIRE_NOTHROW( kerns = queue->get_kernels() );
  for (auto k=kerns->begin(); k!=kerns->end(); ++k) {
    CHECK_NOTHROW( (*k)->split_to_tasks() );
    vector<shared_ptr<task>> tsks; REQUIRE_NOTHROW( tsks = (*k)->get_tasks() );
    for ( auto t : tsks ) {
      if (t->has_type_origin()) {
	auto d = t->get_domain();
	if (d.is_on_face(decomp)) {
  ...
  data_pointer ydata;
  REQUIRE_NOTHROW( ydata = yvector->get_numa_data_pointer() );
  for (index_int i=0; i<nglobal; i++) {
    double s = pointfunc33(i,0);
    if (i>0) s += pointfunc33(i-1,0);
    if (i<nglobal-1) s += pointfunc33(i+1,0);
    INFO( "yvalue[" << i << "] should be " << s << ", not " << ydata->at(i) );
    CHECK( ydata->at(i)==Approx(s) );
  }
}
...
TEST_CASE( "multi-dimensional kernels","[multi][kernel][200]" ) {
  ...
  REQUIRE_NOTHROW( mdecomp = omp_decomposition(arch,layout) );
  processor_coordinate mycoord;
  for (int mytid=0; mytid<ntids; mytid++) {
  ...
  REQUIRE_NOTHROW( d = shared_ptr<distribution>( new omp_block_distribution(mdecomp,domain) ) );
  ...
  shared_ptr<object> o1,r1;
  REQUIRE_NOTHROW( o1 = shared_ptr<object>( new omp_object(d) ) );
  REQUIRE_NOTHROW( r1 = shared_ptr<object>( new omp_object(d) ) );

  shared_ptr<kernel> copy;
  REQUIRE_NOTHROW( copy = shared_ptr<kernel>( new omp_kernel(o1,r1) ) );
  const char *sigma;
  SECTION( "explicit beta" ) { sigma = "explicit beta";
    REQUIRE_NOTHROW( copy->set_last_dependency().set_explicit_beta_distribution(d) );
  }
  ...
  shared_ptr<kernel> make;
  REQUIRE_NOTHROW( make = shared_ptr<kernel>( new omp_origin_kernel(o1) ) );
  REQUIRE_NOTHROW( make->set_localexecutefn(&vecsetconstantp) );
  REQUIRE_NOTHROW( copy_queue->add_kernel(make) );

  REQUIRE_NOTHROW( copy_queue->add_kernel( shared_ptr<kernel>( new omp_origin_kernel(o1) ) ) );
  ...
  for ( auto t : tsks ) {
    ...
    INFO( " origin first index at " << l << "; data = " << data->at(l) );
    REQUIRE( data->at(l)==Approx( (double)(dom.linearize(mdecomp)) ) );
  }
  ...
  for ( auto t : tsks ) {
    ...
    INFO( " input first index at " << l << "; data = " << data->at(l) );
    REQUIRE( data->at(l)==Approx( (double)(dom.linearize(mdecomp)) ) );
  }
  for ( auto t : tsks ) {
    auto dom = t->get_domain();
    ...
    INFO( " halo first index at " << l << "; data = " << data->at(l) );
    REQUIRE( data->at(l)==Approx( (double)(dom.linearize(mdecomp)) ) );
  }
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    processor_coordinate mycoord; data_pointer data;
    REQUIRE_NOTHROW( mycoord = mdecomp.coordinate_from_linear(mytid) );

    REQUIRE_NOTHROW( data = r1->get_data(mycoord) );
    REQUIRE_NOTHROW( l = halo->location_of_first_index(*halo.get(),mycoord) );
    INFO( " output first index at " << l << "; data = " << data->at(l) );
    REQUIRE( data->at(l)==Approx( (double)mytid ) );
  }
}
...
TEST_CASE( "multi-dimensional reverse","[multi][kernel][201]" ) {
  ...
  //index_int g;
  vector<index_int> domain{ntids_i*(nlocal+1),ntids_j*(nlocal+2)};
  ...
  parallel_structure transstruct(mdecomp);
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    REQUIRE_NOTHROW( pflip = processor_coordinate
		     ( vector<int>{layout.coord(0)-mycoord.coord(0)-1,
  ...
  //   shared_ptr<multi_indexstruct> str = transstruct->get_processor_structure(pcoord);
  //   fmt::print("{} struct : {}\n",pcoord.as_string(),str->as_string());
  // }

  shared_ptr<distribution> transdist;
  REQUIRE_NOTHROW( transdist = shared_ptr<distribution>( new omp_distribution(transstruct) ) );
  ...
  REQUIRE_NOTHROW( flip = shared_ptr<kernel>( new omp_kernel(o1,r1) ) );
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
	...
	CHECK( data->at(loc)==Approx((double)mytid) );
      }
    }
  }
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
	...
	index_int loc = INDEX2D( i,j,global_first,numsize );
	INFO( "index " << i << "," << j << " at linear loc " << loc );
	CHECK( data->at(loc)==Approx((double)(ntids-mytid-1) ) );
      }
    }
  }
}
...
TEST_CASE( "two-dimensional transpose","[multi][kernel][202]" ) {
  ...
  for (int mytid=0; mytid<ntids; mytid++) {
    ...
    REQUIRE_NOTHROW( flipcoord = new processor_coordinate( vector<int>{mytid_j,mytid_i} ) );
    INFO( "proc " << mytid << "=" << mycoord.as_string() <<
	  "; flipped " << fliptid << "=" << flipcoord.as_string() );
  }
