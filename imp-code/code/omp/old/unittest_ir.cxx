...
TEST_CASE( "Dump distribution","[distribution][output][6]" ) {
  ...
  REQUIRE_NOTHROW( d = std::shared_ptr<distribution>( new omp_distribution(env,"disjoint-block",500) ) );  
...
TEST_CASE( "dump task","[task][output][8]" ) {
  ...
  REQUIRE_NOTHROW( d1 = std::shared_ptr<distribution>( new omp_distribution(env,"disjoint-block",200) ) );  
...
TEST_CASE( "Threepoint queue with gen kernel","[queue][execute][halo][modulo][10]" ) {
  ...
  auto block = 
    std::shared_ptr<distribution>( new omp_distribution(env,"disjoint-block",nlocal*ntids) );
  ...
  {
    std::shared_ptr<kernel> k = std::shared_ptr<kernel>( new omp_origin_kernel(xvector) );
  ...
  for (int iv=0; iv<nsteps; iv++) {
    ...
    if (iv==0) {
      k = std::shared_ptr<kernel>( new omp_kernel(xvector,yvector[0]) );
    } else {
      k = std::shared_ptr<kernel>( new omp_kernel(yvector[iv-1],yvector[iv]) );
    }
