...
TEST_CASE( "distribution derivation","[1]" ) {
  ...
  int nlocal = 8;
  shared_ptr<distribution> level_dist =
...
TEST_CASE( "center of mass function","[ortho][11]" ) {
  ...
  INFO( "k=" << k );
  REQUIRE_NOTHROW
    ( level_dist = shared_ptr<distribution>( make_shared<mpi_block_distribution>(decomp,k,nlocal,-1) ) );
  ...
  shared_ptr<object> bot,top;
  REQUIRE_NOTHROW( bot = shared_ptr<object>( new mpi_object(level_dist) ) );
  REQUIRE_NOTHROW( bot->allocate() );
  REQUIRE_NOTHROW( top = shared_ptr<object>( new mpi_object(new_dist) ) );
  ...
  if (ipath==0) {
    path = "by pieces";
    REQUIRE_NOTHROW( calculate_cm = shared_ptr<kernel>( new mpi_kernel(bot,top) ) );
    ...
    path = "as one kernel";
    REQUIRE_NOTHROW( calculate_cm = shared_ptr<kernel>( new mpi_centerofmass_kernel(bot,top,k) ) );
  }
  INFO( "path: " << path );
  ...
  for (index_int i=0; i<lsize; i++) {
    ...
    CHECK( data.at(k*i)==g+g+1 );
    if (k>1)
      CHECK( data.at(k*i+1)>first );
  }
}
...
TEST_CASE( "center of mass function, entirely local","[12]" ) {
  ...
  REQUIRE_NOTHROW
    ( level_dist = shared_ptr<distribution>( make_shared<mpi_block_distribution>(decomp,nlocal,-1) ) );
  ...
  shared_ptr<kernel> calculate_cm;
  REQUIRE_NOTHROW( calculate_cm = shared_ptr<kernel>( new mpi_centerofmass_kernel(bot,top) ) );
  REQUIRE_NOTHROW( calculate_cm->analyze_dependencies() );
  ...
  shared_ptr<multi_indexstruct> tststruct;
  ...
  if (recv==1) {
    REQUIRE_NOTHROW
      ( bigstruct = shared_ptr<multi_indexstruct>
	(new multi_indexstruct
	 ( shared_ptr<indexstruct>( new contiguous_indexstruct( 0,bigstruct->local_size(0)-1 ) ) )) );
...
TEST_CASE( "center of mass function, redundant","[13]" ) {
  ...
  REQUIRE_NOTHROW
    ( level_dist = shared_ptr<distribution>( make_shared<mpi_block_distribution>(decomp,1,-1) ) );
  auto coarsen = ioperator(":2");
...
TEST_CASE( "force prolongation","[21]" ) {
  ...
  REQUIRE_NOTHROW( mat = shared_ptr<sparse_matrix>( new mpi_sparse_matrix(level_dist) ) );
  index_int
    f = level_dist->first_index_r(mycoord).coord(0), l = level_dist->last_index_r(mycoord).coord(0),
    g = domain_coordinate( level_dist->global_size() ).at(0);
  ...
  auto bot = shared_ptr<object>( new mpi_object(level_dist) ),
    top = shared_ptr<object>( new mpi_object(new_dist) ),
    side = shared_ptr<object>( new mpi_object(level_dist) ),
    expanded = shared_ptr<object>( new mpi_object(level_dist) ),
    multiplied = shared_ptr<object>( new mpi_object(level_dist) );
  ...
  SECTION( "in pieces" ) {
    ...
    REQUIRE_NOTHROW( expand = shared_ptr<kernel>( new mpi_kernel(top,expanded) ) );
    ...
    REQUIRE_NOTHROW( multiply = shared_ptr<kernel>( new mpi_spmvp_kernel(side,multiplied,mat) ) );
    REQUIRE_NOTHROW( multiply->analyze_dependencies() );
    REQUIRE_NOTHROW( multiply->execute() );
    
    REQUIRE_NOTHROW( calculate_cm = shared_ptr<kernel>( new mpi_sum_kernel(expanded,multiplied,bot) ) );
  }
  SECTION( "as one kernel" ) {
    path = "all in one";
    REQUIRE_NOTHROW
      ( calculate_cm = shared_ptr<kernel>( new mpi_sidewaysdown_kernel(top,side,bot,mat) ) );
  }
  INFO( "path: " << path );
  ...
  for (index_int i=0; i<lsize; i++) {
    ...
    INFO( "ig: " << ig );
    if (ig==0 || ig==g-1 )
      CHECK( data.at(i)==ig/2+1 ); // divide by two because of the expand
    else
      CHECK( data.at(i)==ig/2+2 );
  }
}
...
TEST_CASE( "force prolongation, short","[22]" ) {
  ...
  shared_ptr<sparse_matrix> mat;
  REQUIRE_NOTHROW
    ( mat = shared_ptr<sparse_matrix>( new mpi_toeplitz3_matrix(level_dist,1.,0.,1.) ) );
  ...
  SECTION( "as one kernel" ) {
    path = "all in one";
    REQUIRE_NOTHROW( calculate_cm = shared_ptr<kernel>( new mpi_sidewaysdown_kernel(top,side,bot,mat) ) );
  }
  INFO( "path: " << path );
