...
TEST_CASE( "test overdecomposition","[mpi][05]" ) {
  ...
  REQUIRE_NOTHROW( decomp = mpi_decomposition(arch) );
...
TEST_CASE( "MPI distributions, local stuff","[mpi][distribution][13]" ) {
  ...
  decomposition decomp; shared_ptr<distribution> d1;
  SECTION( "local size" ) {
    SECTION( "regular" ) { over = 1;
  ...
  SECTION( "global size" ) {
    ...
    REQUIRE_NOTHROW( decomp = mpi_decomposition(arch) );
    CHECK( decomp.domains_volume()==over*ntids );
    REQUIRE_NOTHROW( d1 = shared_ptr<distribution>( make_shared<mpi_block_distribution>(decomp,s) ) );
  }
  INFO( "over = " << over );
...
TEST_CASE( "Overdecomposed objects","[object][over][20]" ) {
  ...
  INFO( fmt::format("Using over factor from mpi_arch: {}",over) );
  decomposition edecomp;
  REQUIRE_NOTHROW( edecomp = mpi_decomposition(earch) );
  index_int nlocal = 10;
  shared_ptr<distribution> d;
  REQUIRE_NOTHROW( d = shared_ptr<distribution>( make_shared<mpi_block_distribution>(edecomp,nlocal,-1) ) );
  mpi_object *o;
  ...
  data_pointer dp; vector<processor_coordinate> domains;
  ...
  for ( auto dom : domains ) {
    INFO( fmt::format("proc {}, domain {}, local domain number: {}",
		      mytid,dom.as_string(),edecomp.get_domain_local_number(dom)) );
    data_pointer ptr; 
...
TEST_CASE( "Analyze single interval, exact processor interval","[index][structure][21]" ) {
  ...
  for (auto dom : decomp.get_domains()) {
    CHECK_NOTHROW( mm = dist->messages_for_segment
		   ( 0,dom,self_treatment::INCLUDE,segment,segment ) );
    CHECK( mm.size()==1 );
    m = mm.at(0);
    CHECK( m->get_sender().coord(0)==0 );
    CHECK( m->get_receiver()==dom );
...
TEST_CASE( "Analyze single interval, sub interval","[index][structure][21]" ) {
  ...
  shared_ptr<distribution> dist = shared_ptr<distribution>( new mpi_distribution(decomp) ); 
  auto pstruct = dist->get_dimension_structure(0);
  CHECK_NOTHROW( pstruct->create_from_global_size(over*localsize*ntids) );
  vector<shared_ptr<message>> mm;
  shared_ptr<message> m; shared_ptr<multi_indexstruct> s,segment;
...
TEST_CASE( "Analyze one dependency, right bump","[operate][dependence][22]") {
  ...
  auto alpha = 
    shared_ptr<distribution>( make_shared<mpi_block_distribution>(decomp,-1,over*localsize*ntids) );
...
TEST_CASE( "Analyze one dependency, left bump","[operate][dependence][23]") {
  ...
  for (auto dom : decomp.get_domains()) {
    ...
    CHECK_NOTHROW( mm = alpha->messages_for_segment
		   ( 0,dom,self_treatment::INCLUDE,segment,segment ) );
    auto domnum = decomp.get_domain_local_number(dom);
    if (domnum>0) {
      CHECK( mm.size()==2 );
    } else {
      CHECK( mm.size()==1 );
...
TEST_CASE( "Inserting new domain","[over][domain][30]" ) {
  ...
  shared_ptr<distribution> block,blockplus;
  ...
  REQUIRE_NOTHROW( blockplus = shared_ptr<distribution>
		   ( make_shared<mpi_block_distribution>( block ) ) );
