...
TEST_CASE( "pivot is done only once","[mask][10]" ) {
  std::shared_ptr<distribution> unique = std::shared_ptr<distribution>( make_shared<mpi_block_distribution>(arch,1,-1) );
  ...
  std::shared_ptr<distribution> unique_on0;
  REQUIRE_NOTHROW( unique_on0 = std::shared_ptr<distribution>( new mpi_distribution(unique) ) );
  {
    object *vec1;
  ...
  REQUIRE_NOTHROW( queue->add_kernel( std::shared_ptr<kernel>( new mpi_origin_kernel(vec0) ) ) );

  SECTION( "test0" ) {
    auto k = std::shared_ptr<kernel>( new mpi_kernel(vec0,vec1) );
