...
TEST_CASE( "Dump distribution","[distribution][output][6]" ) {
  ...
  REQUIRE_NOTHROW( d = make_shared<mpi_block_distribution>(arch,500) );  
...
TEST_CASE( "dump task","[task][output][8]" ) {
  std::shared_ptr<distribution> d1,*d2;
  REQUIRE_NOTHROW( d1 = make_shared<mpi_block_distribution>(arch,200) );  
  ...
  task *t; kernel *k;
  REQUIRE_NOTHROW( k = std::shared_ptr<kernel>( new mpi_kernel(o1,o2) ) );
...
TEST_CASE( "Threepoint queue with gen kernel","[queue][execute][halo][modulo][10]" ) {
  ...
  mpi_std::shared_ptr<distribution> block = 
    make_shared<mpi_block_distribution>(arch,nlocal*ntids);
  index_int
  ...
  {
    mpi_kernel *k = std::shared_ptr<kernel>( new mpi_origin_kernel(xvector) );
  ...
  for (int iv=0; iv<nsteps; iv++) {
    mpi_kernel *k;
    if (iv==0) {
      k = std::shared_ptr<kernel>( new mpi_kernel(xvector,yvector[0]) );
    } else {
...
TEST_CASE( "other statistics","[statistics][20]" ) {
  int nlocal=100;
  std::shared_ptr<distribution> blocked;
  REQUIRE_NOTHROW( env->reset_statistics() );
  REQUIRE_NOTHROW( blocked = make_shared<mpi_block_distribution>(arch,nlocal,-1) );
