...
int main(int argc,char **argv) {
  ...
  // Sparse matrix
  std::shared_ptr<sparse_matrix> coefficient_matrix;
  if (sparse) {
    coefficient_matrix =
      std::shared_ptr<sparse_matrix>( new IMP_toeplitz3_matrix(blocked,-1.,2.,-1.) );
  }

  // let's define the steps of the loop body
  algorithm cg = IMP_algorithm(decomp);
  ...
  { auto xorigin = shared_ptr<kernel>( new IMP_origin_kernel( x0 ) ); xorigin->set_name("origin x0");
    ...
    cg.add_kernel(xorigin); }
  { shared_ptr<kernel> borigin;
    if (sparse)
      borigin = std::shared_ptr<kernel>( new IMP_spmvp_kernel( xt,b0,coefficient_matrix ) );
    else
      borigin = std::shared_ptr<kernel>( new IMP_centraldifference_kernel( xt,b0 ) );
    borigin->set_name("b0=A xtrue");
    cg.add_kernel(borigin); }
  if (trace) {
    auto b0norm = std::shared_ptr<object>( new IMP_object(scalar) );
    auto r0inp = shared_ptr<kernel>( new IMP_norm_kernel( b0,b0norm ) );
    cg.add_kernel( r0inp );
    cg.add_kernel( std::shared_ptr<kernel>( new IMP_trace_kernel(b0norm,std::string("A times xtrue")) ) );
  }
  { shared_ptr<kernel> atimesx0;
    if (sparse)
      atimesx0 = std::shared_ptr<kernel>( new IMP_spmvp_kernel( x0,ax0,coefficient_matrix ) );
    else
      atimesx0 = std::shared_ptr<kernel>( new IMP_centraldifference_kernel( x0,ax0 ) );
  ...
  if (trace) {
    auto rr0 = std::shared_ptr<object>( new IMP_object(scalar) );
    auto r0inp = shared_ptr<kernel>( new IMP_norm_kernel( r0,rr0 ) );
    cg.add_kernel( r0inp );
    cg.add_kernel( std::shared_ptr<kernel>( new IMP_trace_kernel(rr0,std::string("Initial residual norm")) ) );
  }
  ...
  for (int it=0; it<n_iterations; it++) {
    ...
    if (sparse)
      matvec = std::shared_ptr<kernel>( new IMP_spmvp_kernel( pcarry,q,coefficient_matrix ) );
    else
      matvec = std::shared_ptr<kernel>( new IMP_centraldifference_kernel( pcarry,q ) );
