 ...
 ****
 **** laplace-4thorder : attempt at 9 point stencil
 ****
 ****************************************************************/
...
int main(int argc,char **argv) {
  ...
  IMP_shared_ptr<object> nodes_in,nodes_out;
  ...
  try {
    ...
    /* Create the objects */
    nodes_in = shared_ptr<object>( new IMP_object(nodes_dist) ); nodes_in->set_name("nodes in");
    nodes_out = shared_ptr<object>( new IMP_object(nodes_dist) ); nodes_out->set_name("nodes out");
    ...
    bilinear_op = shared_ptr<kernel>( new IMP_kernel(nodes_in,nodes_out) );
    ...
    algorithm *bilinear = shared_ptr<algorithm>( new IMP_algorithm(decomp) );
    bilinear->add_kernel( shared_ptr<kernel>( new IMP_origin_kernel(nodes_in) ) );
