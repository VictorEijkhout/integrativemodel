...
int main(int argc,char **argv) {
  ...
  auto
    blocked = shared_ptr<distribution>( new IMP_block_distribution(decomp,nglobal) ),
    scalar = shared_ptr<distribution>( new IMP_replicated_distribution(decomp) );
  auto xs = new std::vector<shared_ptr<object>>;
  auto ys = new std::vector<shared_ptr<object>>;
  for (int step=0; step<=nsteps; step++) {
    shared_ptr<object> step_object,out_object;
    if (reuse && step>0) {
      xs->at(0)->allocate();
      ys->at(0)->allocate();
      step_object = shared_ptr<object>( new IMP_object(blocked,xs->at(0)->get_raw_data()) );
      out_object = shared_ptr<object>( new IMP_object(blocked,ys->at(0)->get_raw_data()) );
    } else {
      step_object = shared_ptr<object>( new IMP_object(blocked) );
      out_object = shared_ptr<object>( new IMP_object(blocked) );
    }
  ...
  algorithm heat = IMP_algorithm(decomp);

  auto initialize = shared_ptr<kernel>( new IMP_origin_kernel(xs->at(0)) );
  domain_coordinate *deltaloc = new domain_coordinate( std::vector<index_int>{0} );
  initialize->set_localexecutefn
    ( [deltaloc] ( kernel_function_args ) -> void {
      vecdelta(kernel_function_call,*deltaloc); } );
  heat.add_kernel( initialize );
  for (int step=0; step<nsteps; step++) {
    heat.add_kernel
      ( shared_ptr<kernel>( new IMP_diffusion_kernel( xs->at(step),ys->at(step) ) ) );
    if (trace) {
      shared_ptr<object> nrm = shared_ptr<object>( new IMP_object(scalar) );
      auto xnrm = shared_ptr<kernel>( new IMP_norm_kernel( xs->at(step+1),nrm ) );
      heat.add_kernel( xnrm );
      heat.add_kernel( shared_ptr<kernel>( new IMP_trace_kernel(nrm,fmt::format("Norm at step {}",step+1) )) );
    }
    heat.add_kernel
      ( shared_ptr<kernel>( new IMP_copy_kernel( ys->at(step),xs->at(step+1) ) ) );
  }
