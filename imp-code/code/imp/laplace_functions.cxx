...
void laplace_bilinear_fn( kernel_function_args ) {
  ...
  const domain_coordinate
    &out_gfirst = out_gstruct->first_index_r(),
    &out_glast = out_gstruct->last_index_r(),
  ...
  //snippet laplacebil
	...
	if (!skip_first_i && !skip_first_j && !skip_last_i && !skip_last_j)
	  ...
	  // center
	  8 *
	  indata.at( INDEX2D(i,j,in_offsets,in_nsize) )
	  +
	  // up
	  -1 *
          ( !skip_first_i 
            ? indata.at( INDEX2D(i-1,j,in_offsets,in_nsize) ) : 0 )
	  +
	  // dn
	  -1 *
          ( !skip_last_i 
	  ...
	  -1 *
          ( !skip_first_j
            ? indata.at( INDEX2D(i,j-1,in_offsets,in_nsize) ) : 0 )
	  +
	  // rt
	  -1 *
          ( !skip_last_j
	  ...
	  -1 *
          ( !skip_first_i && !skip_first_j
            ? indata.at( INDEX2D(i-1,j-1,in_offsets,in_nsize) ) : 0 )
          +
	  // ru
	  -1 *
