...
TEST_CASE( "contiguous indexstruct","[indexstruct][1]" ) {
  ...
  shared_ptr<indexstruct> i1,i2,i3,i4;
  ...
  SECTION( "basic construction" ) {
    ...
    SECTION( "by accretion" ) {
      i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(2,5) );
    ...
    SECTION( "more contiguous" ) {
      i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(1,7) );
      CHECK( i1->is_contiguous() );
      CHECK( !i1->is_indexed() );
      CHECK( i1->first_index()==1 );
    ...
    SECTION( "strided" ) {
      i1 = shared_ptr<indexstruct>( new strided_indexstruct(2,6,2) );
      CHECK( i1->is_strided() );
      CHECK( !i1->is_contiguous() );
      CHECK( !i1->is_indexed() );
      CHECK( i1->first_index()==2 );
  ...
  SECTION( "striding and operations" ) {
    i1 = shared_ptr<indexstruct>{ new strided_indexstruct(4,7,2) };
    ...
    SECTION( "basic stride tests" ) {
      ...
      i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(4,6) );
      CHECK( !i1->equals(i2) );
      CHECK( !i2->equals(i1) );
      CHECK( i2->get_ith_element(1)==5 );
      CHECK_THROWS( i2->get_ith_element(3) );

      i2 = shared_ptr<indexstruct>( new strided_indexstruct(4,6,2) );
      CHECK( i1->equals(i2) );
      CHECK( i2->equals(i1) );
    ...
    SECTION( "strided containment" ) {
      i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(4,8) );
      i3 = shared_ptr<indexstruct>( new strided_indexstruct(4,8,2) );
      i4 = shared_ptr<indexstruct>( new strided_indexstruct(4,8,4) );
  ...
  SECTION( "copy indexstruct" ) {
    ...
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(7,15) };
  ...
  SECTION( "find in contiguous" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(8,12) };
    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(10,12,2) };
    index_int loc;
  ...
  SECTION( "find in strided" ) {
    i1 = shared_ptr<indexstruct>{ new strided_indexstruct(8,12,2) };
    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(10,12,2) };
    i3 = shared_ptr<indexstruct>{ new strided_indexstruct(11,12,2) };
    index_int loc;
    REQUIRE_NOTHROW( loc = i1->location_of(i2) );
    CHECK( loc==1 );
...
TEST_CASE( "indexed indexstruct","[indexstruct][2]" ) {
  ...
  SECTION( "basic construction" ) {
    SECTION( "correct" ) {
      ...
      i1 = shared_ptr<indexstruct>( new indexed_indexstruct(len,idx) );
      CHECK( !i1->is_contiguous() );
      CHECK( i1->is_indexed() );
      CHECK( i1->first_index()==1 );
      CHECK( i1->last_index()==4 );
    ...
    SECTION( "unsorted throws an error" ) {
      ...
      CHECK_THROWS( i1 = shared_ptr<indexstruct>( new indexed_indexstruct(len,idx) ) );
    }
    ...
    SECTION( "gradual construction" ) {
      ...
      auto i1 = shared_ptr<indexstruct>( new indexed_indexstruct(len,idx) );
      CHECK( i1->local_size()==3 );
      CHECK( i1->first_index()==4 );
	...
	( new indexed_indexstruct( vector<index_int>{2,6,8} ) ),
	i2 = 
	shared_ptr<indexstruct>
	( new indexed_indexstruct( vector<index_int>{3,5,7} ) );
      ...
      i1 = shared_ptr<indexstruct>( new empty_indexstruct() );
      REQUIRE_NOTHROW( i2 = i1->force_simplify() );
      i1 = shared_ptr<indexstruct>( new empty_indexstruct() );
      REQUIRE_NOTHROW( i1 = i1->struct_union
		       ( shared_ptr<indexstruct>( new empty_indexstruct() ) ) );
      REQUIRE_NOTHROW( i2 = i1->force_simplify() );
    }
  }
  ...
  SECTION( "more simplify" ) {
    auto i1 =
      shared_ptr<indexstruct>
      ( new indexed_indexstruct( vector<index_int>{2,4, 10,12, 15} ) );
  ...
  SECTION( "striding and operations" ) {
    ...
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>{ new indexed_indexstruct(len,idx) } );
  ...
  SECTION( "find in indexed" ) {
    ...
    auto
      i2 = shared_ptr<indexstruct>( new strided_indexstruct(2,4,2) ),
      i3 = shared_ptr<indexstruct>( new strided_indexstruct(7,8,2) );
    index_int loc;
    REQUIRE_NOTHROW( loc = i1->location_of(i2) );
    CHECK( loc==1 );
...
TEST_CASE( "composite indexstruct","[indexstruct][composite][8]" ) {
  shared_ptr<indexstruct> i1,i2,ifinal;
  shared_ptr<composite_indexstruct> icomp;

  i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(3,5) };
  i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(10,12) };
  SECTION( "catch old design: obviated" ) {
    REQUIRE_NOTHROW( icomp = shared_ptr<composite_indexstruct>{ new composite_indexstruct() } );
  ...
  SECTION( "tricky composite simplify with indexed" ) {
    auto i1 = shared_ptr<composite_indexstruct>( new composite_indexstruct() );
    auto left_cont = shared_ptr<indexstruct>( new contiguous_indexstruct(0,9) );
    REQUIRE_NOTHROW( i1->push_back(left_cont) );
    auto right_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(11,19) );
    REQUIRE_NOTHROW( i1->push_back(right_cont) );
    auto more_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(23,30) );
    REQUIRE_NOTHROW( i1->push_back(more_cont) );
    auto gaps = shared_ptr<indexstruct>
      ( new indexed_indexstruct( vector<index_int>{10,20,22,40} ) );
  ...
  SECTION( "tricky composite simplify with strided" ) {
    ...
    auto right_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(11,19) );
    REQUIRE_NOTHROW( i1->push_back(right_cont) );
    auto more_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(23,29) );
    REQUIRE_NOTHROW( i1->push_back(more_cont) );
  ...
  SECTION( "composite over simplify" ) {
    ...
    auto right_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(11,19) );
    REQUIRE_NOTHROW( i1->push_back(right_cont) );
    auto left_cont = shared_ptr<indexstruct>( new contiguous_indexstruct(2,9) );
    REQUIRE_NOTHROW( i1->push_back(left_cont) );
    SECTION( "just two members" ) {
    }
    SECTION( "three can also be incorporated" ) {
      SECTION( "contiguous" ) {
	auto more_cont =shared_ptr<indexstruct>( new contiguous_indexstruct(21,29) );
	REQUIRE_NOTHROW( i1->push_back(more_cont) );
      }
      SECTION( "indexed" ) {
	auto more_cont =shared_ptr<indexstruct>( new indexed_indexstruct( vector<index_int>{21} ) );
	REQUIRE_NOTHROW( i1->push_back(more_cont) );
      }
    }
    SECTION( "index" ) {
      has_index = true;
      auto more =shared_ptr<indexstruct>( new indexed_indexstruct( vector<index_int>{0,40,100} ) );
...
TEST_CASE( "enumerating indexstructs","[10]" ) {
  ...
  SECTION( "contiguous" ) {
    ...
    REQUIRE_NOTHROW( idx = shared_ptr<indexstruct>( new contiguous_indexstruct(13,15) ) );
  ...
  SECTION( "strided" ) {
    shared_ptr<indexstruct> idx;
    REQUIRE_NOTHROW( idx = shared_ptr<indexstruct>( new strided_indexstruct(3,10,2) ) );
  ...
  SECTION( "indexed" ) {
    ...
    shared_ptr<indexstruct> idx;
    REQUIRE_NOTHROW( idx = shared_ptr<indexstruct>( new indexed_indexstruct(4,ar) ) );
    count = 0;
    SECTION( "traditional" ) {
      for (auto i=idx->begin(); i!=idx->end(); ++i) {
	CHECK( *i==ar[count] );
  ...
  SECTION( "composite" ) {
    auto
      i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(1,10) ),
      i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(21,30) );
    shared_ptr<indexstruct> icomp;
    REQUIRE_NOTHROW( icomp = shared_ptr<indexstruct>{ i1->make_clone() } );
...
TEST_CASE( "indexstruct intersections","[indexstruct][intersect][20]" ) {
  ...
  SECTION( "first cont" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(1,10) };
    indexstructure I1( contiguous_indexstruct(1,10) );
    SECTION( "cont-cont" ) {
      i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(5,12) };
      ...
      indexstructure I2( contiguous_indexstruct(5,12) ), I3;
      REQUIRE_NOTHROW( I3 = I1.intersect(I2) );
      CHECK( I3.is_contiguous() );
      CHECK( I3.first_index()==5 );
      CHECK( I3.last_index()==10 );
      ...
      i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(11,12) };
      REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
      REQUIRE( i3->is_empty() );

      i2 = shared_ptr<indexstruct>{ new strided_indexstruct(10,12,2) };
      i3 = shared_ptr<indexstruct>( new strided_indexstruct(8,14,2) );
    ...
    SECTION( "cont-idx" ) {
      ...
      i2 = shared_ptr<indexstruct>{ new indexed_indexstruct(len,idx) };
      REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
      REQUIRE( !i3->is_empty() );
      CHECK( i3->is_indexed() );
      ...
      i2 = shared_ptr<indexstruct>{ new indexed_indexstruct(len,idxs) };
  ...
  SECTION( "stride-stride" ) {
    i1 = shared_ptr<indexstruct>{ new strided_indexstruct(10,20,2) };
    CHECK( i1->local_size()==6 );
    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(12,14,2) };
    REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
    CHECK( i3->local_size()==2 );
    CHECK( i3->is_strided() );
    CHECK( i3->stride()==2 );

    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(12,22,10) };
    REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
    CHECK( i3->local_size()==1 );

    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(12,20,4) };
    REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
    CHECK( i3->local_size()==3 );
    CHECK( i3->is_strided() );

    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(12,20,5) };
    REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
    CHECK( i3->local_size()==1 );

    i2 = shared_ptr<indexstruct>{ new strided_indexstruct(13,20,4) };
    REQUIRE_NOTHROW( i3 = i1->intersect(i2) );
    CHECK( i3!=nullptr );
    CHECK( i3->is_empty() );
  }
  SECTION( "strided-indexed" ) {
    i1 = shared_ptr<indexstruct>{ new strided_indexstruct(10,20,10) };
    CHECK( i1->local_size()==2 );
    i2 = shared_ptr<indexstruct>{
      new indexed_indexstruct( vector<index_int>{10,16,20} ) };
  ...
  SECTION( "idx-idx" ) {
    shared_ptr<indexstruct> i5{nullptr};
    i2 = shared_ptr<indexstruct>{
      new indexed_indexstruct( vector<index_int>{4,8,11} ) };
    i3 = shared_ptr<indexstruct>{
      new indexed_indexstruct( vector<index_int>{3,8,10,11,12} ) };
...
TEST_CASE( "indexstruct differences","[indexstruct][minus][21]" ) {
  indexstruct *it;
  shared_ptr<indexstruct> i1,i2,i3;
  SECTION( "cont-cont non-overlapping" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(5,15) );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(20,30) );
    SECTION( "one way" ) {
  ...
  SECTION( "cont-cont containment" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(5,30) );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(15,20) );
    SECTION( "one way" ) {
      REQUIRE_NOTHROW( i3 = i1->minus(i2) );
  ...
  SECTION( "cont-cont for real" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(5,20) );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(15,30) );
    SECTION( "one way" ) {
      REQUIRE_NOTHROW( i3 = i1->minus(i2) );
      INFO( i3->as_string() );
      CHECK( i3->is_contiguous() );
  ...
  SECTION( "contiguous minus contiguous, creating gap" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(1,40) );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(11,20) );
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
  ...
  SECTION( "strided minus contiguous, creating gap" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct(1,41,4) );
    CHECK( i1->local_size()==11 );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(12,16) ); // this only cuts 1
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
    INFO( "resulting i3: " << i3->as_string() );
    CHECK( i3->is_composite() );
  ...
  SECTION( "strided minus contiguous, hitting nothing" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct(1,41,4) );
    CHECK( i1->local_size()==11 );
    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(14,16)); // this hits nothing: falls between 13-17
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
    INFO( "resulting i3: " << i3->as_string() );
  ...
  SECTION( "indexed cont" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct( 4,16,4 ) );
    i1 = i1->convert_to_indexed() ;
    CHECK( i1->is_indexed() );
    CHECK( i1->first_index()==4 );
    CHECK( i1->last_index()==16 );
    i2  = shared_ptr<indexstruct>( new contiguous_indexstruct( 13,17 ) );
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
    CHECK( i3->is_indexed() );
    CHECK( i3->first_index()==4 );
    CHECK( i3->last_index()==12 );

    indexstructure I1(strided_indexstruct( 4,16,4 )), I2(i2), I3;
    REQUIRE_NOTHROW( I3 = I1.minus(I2) );
    CHECK( I3.local_size()==3 );
    CHECK( I3.first_index()==4 );
    CHECK( I3.last_index()==12 );
  ...
  SECTION( "indexed-indexed" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct( 4,16,4 ) );
    i1 = shared_ptr<indexstruct>( i1->convert_to_indexed() );
    i2 = shared_ptr<indexstruct>( new strided_indexstruct( 5,17,4 ) );
    i2 = shared_ptr<indexstruct>( i2->convert_to_indexed() );
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
    CHECK( i3->equals(i1) );
    i2 = shared_ptr<indexstruct>( new strided_indexstruct( 16,20,2 ) );
    i2 = shared_ptr<indexstruct>( i2->convert_to_indexed() );
    REQUIRE_NOTHROW( i3 = i1->minus(i2) );
    CHECK( i3->local_size()==3 );
    CHECK( i3->first_index()==4 );
...
TEST_CASE( "indexstruct unions","[indexstruct][union][22]" ) {
  ...
  SECTION( "convert from stride 1" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(2,2+SMALLBLOCKSIZE-1) };
    CHECK( i1->is_contiguous() );
  ...
  SECTION( "convert from stride 2" ) {
    i1 = shared_ptr<indexstruct>{ new strided_indexstruct(2,2+2*SMALLBLOCKSIZE-2,2) };
    CHECK_NOTHROW( i2 = i1->convert_to_indexed() );
    CHECK( i2->is_indexed() );
    CHECK( i2->first_index()==2 );
  ...
  SECTION( "cont-cont" ) {
    ...
    SECTION( "2" ) {
      i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(11,13) };
      REQUIRE_NOTHROW( i3 = i1->struct_union(i2) );
      REQUIRE( i3!=nullptr );
      CHECK( i3->is_contiguous() );
      CHECK( i3->first_index()==1 );
	...
	i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(1,10) };
	i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(13) };
	...
	CHECK( i3->last_index()==13 );
      ...
      SECTION( "extend left" ) {
	i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(1,10) };
	i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(14) };
  ...
  SECTION( "cont-idx giving indexed" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(5,8) };
    i2 = shared_ptr<indexstruct>( new strided_indexstruct(8,12,2) );
    CHECK( i2->local_size()==3 );
  ...
  SECTION( "cont-idx extending" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(5,11) };
    i2 = shared_ptr<indexstruct>( new strided_indexstruct(8,12,2) );
    indexstructure I3,
      I1(contiguous_indexstruct(5,11)),
      I2(strided_indexstruct(8,12,2));
    CHECK( i2->local_size()==3 );
    REQUIRE_NOTHROW( i2 = i2->convert_to_indexed() );
    CHECK( i2->is_indexed() );
    CHECK( i2->local_size()==3 );
  ...
  SECTION( "cont-idx extending2" ) {
    i1 = shared_ptr<indexstruct>{ new contiguous_indexstruct(5,11) };
    i2 = shared_ptr<indexstruct>( new strided_indexstruct(6,12,2) );
  ...
  SECTION( "idx-cont" ) {
    ...
    i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(5,8) };
    REQUIRE_NOTHROW( i3 = i1->struct_union(i2) );
    CHECK( i3->local_size()==6 );
    CHECK( i3->first_index()==5 );
    CHECK( i3->last_index()==12);
  ...
  SECTION( "tricky composite stuff" ) {
    shared_ptr<composite_indexstruct> icomp;
    REQUIRE_NOTHROW
      ( icomp = shared_ptr<composite_indexstruct>{ new composite_indexstruct() } );
    CHECK( icomp->is_composite() );
    REQUIRE_NOTHROW
      ( icomp->push_back( shared_ptr<indexstruct>( new contiguous_indexstruct(1,10) ) ) );
    REQUIRE_NOTHROW
      ( icomp->push_back( shared_ptr<indexstruct>( new contiguous_indexstruct(31,40) ) ) );
    i1 = icomp->make_clone();
    CHECK( i1->is_composite() );
    vector< shared_ptr<indexstruct> > members;
    indexstructure I1;
    REQUIRE_NOTHROW( I1 = indexstructure(composite_indexstruct()) );
    CHECK( I1.is_composite() );
    REQUIRE_NOTHROW( I1.push_back( contiguous_indexstruct(1,10) ) );
    return;
    REQUIRE_NOTHROW( I1.push_back( contiguous_indexstruct(31,40) ) );
    SECTION( "can not merge" ) {
      i2 = shared_ptr<indexstruct>{ new strided_indexstruct(11,15,2) };
    ...
    SECTION( "can merge" ) {
      i2 = shared_ptr<indexstruct>{ new contiguous_indexstruct(11,15) };
      REQUIRE_NOTHROW( i1 = i1->struct_union(i2) );
      REQUIRE_NOTHROW( members = dynamic_cast<composite_indexstruct*>(i1.get())->get_structs() );
...
TEST_CASE( "struct disjoint","[23]" ) {
  ...
  indexstructure I1,I2;
  SECTION( "disjoint strided" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>( new strided_indexstruct(1,10,2) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) ) );
    CHECK( i1->disjoint(i2) );
    REQUIRE_NOTHROW( I1 = indexstructure(strided_indexstruct(1,10,2)) );
    REQUIRE_NOTHROW( I2 = indexstructure(contiguous_indexstruct(10,20)) );
    CHECK( I1.disjoint(I2) );
  }
  SECTION( "disjoint strided, interleaved" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>( new strided_indexstruct(1,10,2) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>( new strided_indexstruct(8,20,2) ) );
    CHECK( i1->disjoint(i2) );
  }
  SECTION( "disjoint strided, hard to tell" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>( new strided_indexstruct(0,10,5) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>( new strided_indexstruct(7,9,2) ) );
    CHECK_THROWS( i1->disjoint(i2) );
  }
  SECTION( "disjoint indexed & strided, range" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>
		     ( new indexed_indexstruct(vector<index_int>{1,3,6}) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(7,10) ) );
  ...
  SECTION( "disjoint indexed, range" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>
		     ( new indexed_indexstruct(vector<index_int>{1,3,6}) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>
		     ( new indexed_indexstruct(vector<index_int>{7,8,10}) ) );
    CHECK( i1->disjoint(i2) );
  }
  SECTION( "disjoint indexed, hard to tell" ) {
    REQUIRE_NOTHROW( i1 = shared_ptr<indexstruct>
		     ( new indexed_indexstruct(vector<index_int>{1,3,16}) ) );
    REQUIRE_NOTHROW( i2 = shared_ptr<indexstruct>
		     ( new indexed_indexstruct(vector<index_int>{7,8,10}) ) );
    CHECK_THROWS( i1->disjoint(i2) );
    REQUIRE_NOTHROW( I1 = indexstructure(indexed_indexstruct(vector<index_int>{1,3,16})) );
    REQUIRE_NOTHROW( I2 = indexstructure(indexed_indexstruct(vector<index_int>{7,8,10})) );
    CHECK_THROWS( I1.disjoint(I2) );
  }
}
...
TEST_CASE( "struct containment","[24]" ) {
  shared_ptr<indexstruct> i1,i2,i3;
  indexstructure I1,I2,I3;
  SECTION( "cont" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(1,10) );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(2,10) );
    REQUIRE( i1->contains(i2) );
    REQUIRE( !i2->contains(i1) );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(2,11) );
    REQUIRE( !i1->contains(i2) );
    REQUIRE( !i2->contains(i1) );

    i2 = shared_ptr<indexstruct>( new strided_indexstruct(3,11,3) ); // 3,6,9
    REQUIRE( i1->contains(i2) );
    REQUIRE( !i2->contains(i1) );

    i2 = shared_ptr<indexstruct>( new strided_indexstruct(4,11,3) ); // 4,7,10
    REQUIRE( i1->contains(i2) );
    REQUIRE( !i2->contains(i1) );
  }
  SECTION( "stride" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct(10,20,3) );
  ...
  SECTION( "idx" ) {
    ...
    i2 = shared_ptr<indexstruct>( new strided_indexstruct(4,4,3) );
    REQUIRE( i1->contains(i2) );

    i2 = shared_ptr<indexstruct>( new strided_indexstruct(4,6,2) );
    REQUIRE( i1->contains(i2) );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(4,4) );
    REQUIRE( i1->contains(i2) );
  }
  SECTION( "composite" ) {
    auto i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(0,10) );
    REQUIRE_NOTHROW( i1 = i1->struct_union
		     ( shared_ptr<indexstruct>( new contiguous_indexstruct(20,29) ) ) );
    CHECK( i1->is_composite() );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(2,5) );
    CHECK( i1->contains(i2) );

    I1 = indexstructure(contiguous_indexstruct(0,10));
    I1 = I1.struct_union(indexstructure(contiguous_indexstruct(20,29)));
    I2 = indexstructure(contiguous_indexstruct(2,5));
    CHECK( I1.contains(I2) );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(22,25) );
    CHECK( i1->contains(i2) );
    I2 = indexstructure(contiguous_indexstruct(22,25));
    CHECK( I1.contains(I2) );

    i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(8,25) );
    CHECK( !i1->contains(i2) );
    I2 = indexstructure(contiguous_indexstruct(8,25));
    CHECK( !I1.contains(I2) );
...
TEST_CASE( "struct split","[split][25]" ) {
  SECTION( "contiguous" ) {
    auto i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) );
    indexstructure I1(contiguous_indexstruct(10,20));
    shared_ptr<indexstruct> c;
    SECTION( "non intersect" ) {
      auto i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(30,40) );
      REQUIRE_THROWS( c = i1->split(i2) );
    }
    SECTION( "contains" ) {
      auto i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(0,40) );
      REQUIRE_THROWS( c = i1->split(i2) );
    }
    SECTION( "right" ) {
      auto i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(15,30) );
      REQUIRE_NOTHROW( c = i1->split(i2) );
      composite_indexstruct *cc = dynamic_cast<composite_indexstruct*>(c.get());
      CHECK( cc!=nullptr );
      auto ss = cc->get_structs();
      CHECK( ss.size()==2 );
      auto l = shared_ptr<indexstruct>( new contiguous_indexstruct(10,14) );
      CHECK( ss.at(0)->equals(l) );
      auto r = shared_ptr<indexstruct>( new contiguous_indexstruct(15,20) );
      CHECK( ss.at(1)->equals(r) );
    }
    SECTION( "left" ) {
      auto i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(0,15) );
      REQUIRE_NOTHROW( c = i1->split(i2) );
      composite_indexstruct *cc = dynamic_cast<composite_indexstruct*>(c.get());
      CHECK( cc!=nullptr );
      auto ss = cc->get_structs();
      CHECK( ss.size()==2 );
      auto l = shared_ptr<indexstruct>( new contiguous_indexstruct(10,15) );
      CHECK( ss.at(0)->equals(l) );
      auto r = shared_ptr<indexstruct>( new contiguous_indexstruct(16,20) );
      CHECK( ss.at(1)->equals(r) );
    }
  }
  SECTION( "strided" ) {
    auto i1 = shared_ptr<indexstruct>( new strided_indexstruct(10,20,2) );
    indexstructure I1(strided_indexstruct(10,20,2));
    shared_ptr<indexstruct> c;
    SECTION( "non intersect" ) {
      auto i2 = shared_ptr<indexstruct>( new contiguous_indexstruct(30,40) );
      indexstructure I2(contiguous_indexstruct(30,40)), c;
      REQUIRE_THROWS( c = i1->split(i2) );
      REQUIRE_THROWS( c = I1.split(I2) );
    }
    SECTION( "contains" ) {
    ...
    SECTION( "right" ) {
      ...
      CHECK( ss.size()==2 );
      {
	auto l = shared_ptr<indexstruct>( new strided_indexstruct(10,14,2) );
	CHECK( ss.at(0)->equals(l) );
      }
      {
	auto r = shared_ptr<indexstruct>( new strided_indexstruct(16,20,2) );
...
TEST_CASE( "structs and operations","[indexstruct][operate][30]" ) {
  ...
  SECTION( "multiply by constant" ) {
    i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(5,10) );
    I1 = indexstructure(contiguous_indexstruct(5,10));
  ...
  SECTION( "multiply range by constant" ) {
    ...
    // CHECK( i2->is_strided() );
    CHECK( i2->stride()==1 );
    REQUIRE_NOTHROW( I1 = indexstructure(i1) );
    REQUIRE_NOTHROW( I2 = I1.operate(op) );
    CHECK( I2.is_contiguous() );
    CHECK( I2.last_index()==21 );
  }
  ...
  SECTION( "shift strided" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct(1,10,2) );
    CHECK( i1->first_index()==1 );
    CHECK( i1->last_index()==9 );
    CHECK( i1->local_size()==5 );
    SECTION( "bump" ) {
  ...
  SECTION( "shift strided with truncate" ) {
    i1 = shared_ptr<indexstruct>( new strided_indexstruct(0,10,2) );
    REQUIRE_NOTHROW( op = ioperator("<=1") );
...
TEST_CASE( "division operation","[indexstruct][operate][31]" ) {
  shared_ptr<indexstruct> i1,i2; ioperator op;
  ...
  SECTION( "simple division" ) {
    ...
    SECTION( "contiguous2" ) {
      i1 = shared_ptr<indexstruct>( new contiguous_indexstruct(0,9) );
      REQUIRE_NOTHROW( i2 = i1->operate(op) );
      CHECK( i2->first_index()==0 );
...
TEST_CASE( "sigma operator stuff","[50]" ) {
  ...
  ioperator times2("*2");
  auto cont = shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) );
  shared_ptr<indexstruct> sstruct; const char *path;
  ...
  SECTION( "struct" ) { path = "operate by struct";
    REQUIRE_NOTHROW
      ( sop = sigma_operator
	( [times2] ( const indexstruct &i) -> shared_ptr<indexstruct>
  ...
  indexstructure Cont,Sstruct;
  REQUIRE_NOTHROW( Cont = indexstructure(cont) );
  REQUIRE_NOTHROW( Sstruct = Cont.operate(sop) );
  CHECK( Sstruct.first_index()==20 );
  CHECK( Sstruct.last_index()==40 );
}

TEST_CASE( "create multidimensional by component","[multi][indexstruct][100]" ) {
  shared_ptr<multi_indexstruct> mi,mt;
  multi_indexstructure Mi,Mt;

  // we can not create zero-dimensional
  REQUIRE_THROWS( mi = shared_ptr<multi_indexstruct>( new multi_indexstruct(0) ) );
  REQUIRE_THROWS( Mi = multi_indexstructure(0) );
  // create two-dimensional
  REQUIRE_NOTHROW( mi = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) ) );
  REQUIRE_NOTHROW( Mi = multi_indexstructure(2) );
  // set two component
  REQUIRE_NOTHROW( mi->set_component
		   ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(2,10) ) ) );
  REQUIRE_NOTHROW( mi->set_component
		   ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(40,41) ) ) );
  CHECK( mi->volume()==9*2 );
  // can set twice
  REQUIRE_NOTHROW( mi->set_component
		   ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(30,41) ) ) );
  CHECK( mi->volume()==9*12 );
  // can not set outside dimension bounds
  REQUIRE_THROWS( mi->set_component
		  ( 2, shared_ptr<indexstruct>( new contiguous_indexstruct(40,41) ) ) );
  CHECK( mi->volume()==9*12 );
  ...
  mt = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) );
  mt->set_component(0,shared_ptr<indexstruct>( new contiguous_indexstruct(4,9)) );
  mt->set_component(1,shared_ptr<indexstruct>( new contiguous_indexstruct(40,40)) );
  REQUIRE_NOTHROW( val = mi->location_of(mt) );
  CHECK( val.at(0)==2 );
  CHECK( val.at(1)==10 );
}

TEST_CASE( "create multidimensional block","[multi][indexstruct][101]" ) {
  int dim = 2; index_int nlocal = 10,v=1;
  multi_indexstruct m(dim);
  vector<index_int> sizes(dim);
  for (int id=0; id<dim; id++) {
    sizes[id] = nlocal+id; v *= nlocal+id;
  }
  SECTION( "from sizes" ) {
    m = multi_indexstruct(sizes);
  }
  SECTION( "from structs" ) {
    vector<shared_ptr<indexstruct>> blocks(dim);
    for (int id=0; id<dim; id++)
      blocks[id] = shared_ptr<indexstruct>( new contiguous_indexstruct(0,nlocal+id-1) );
    m = multi_indexstruct(blocks);
  }
  REQUIRE( m.volume()==v );
}

TEST_CASE( "multidimensional containment","[multi][111]" ) {
  shared_ptr<multi_indexstruct> m1,m1a,m2,m2a;
  SECTION( "simple:" ) {
    REQUIRE_NOTHROW
      ( m1 = shared_ptr<multi_indexstruct>( new multi_indexstruct
	( vector<shared_ptr<indexstruct>>{
	  shared_ptr<indexstruct>( new contiguous_indexstruct(0,9) ) } ) ) );
    INFO( "m1:" << m1->as_string() );
    SECTION( "contains simple" ) {
      REQUIRE_NOTHROW
	( m2 = shared_ptr<multi_indexstruct>( new multi_indexstruct
	  ( vector<shared_ptr<indexstruct>>{
	    shared_ptr<indexstruct>( new contiguous_indexstruct(1,5) ) } ) ) );
	...
	( m2 = shared_ptr<multi_indexstruct>( new multi_indexstruct
	  ( vector<shared_ptr<indexstruct>>{
	    shared_ptr<indexstruct>( new contiguous_indexstruct(1,15) ) } ) ) );
      INFO( "m2:" << m2->as_string() );
      CHECK( !m1->contains(m2) );
    }
  }
  SECTION( "multi:" ) {
	...
	( vector<shared_ptr<indexstruct>>{
	  shared_ptr<indexstruct>( new contiguous_indexstruct(0,9) ) } ) ) );
    REQUIRE_NOTHROW
      ( m1a = shared_ptr<multi_indexstruct>( new multi_indexstruct
	( vector<shared_ptr<indexstruct>>{
	  shared_ptr<indexstruct>( new contiguous_indexstruct(20,29) ) } )
						  ) );
	...
	( m2 = shared_ptr<multi_indexstruct>( new multi_indexstruct
	  ( vector<shared_ptr<indexstruct>>{
	    shared_ptr<indexstruct>( new contiguous_indexstruct(5,25) ) } ) ) );
      INFO( "m2:" << m2->as_string() );
      CHECK( !m1->contains(m2) );
    }
  }
...
TEST_CASE( "multidimensional union","[multi][union][indexstruct][113]" ) {
  shared_ptr<multi_indexstruct> mi1,mi2,mi3; int diff;
  REQUIRE_NOTHROW( mi1 = shared_ptr<multi_indexstruct>( new multi_indexstruct(1) ) );
  REQUIRE_NOTHROW( mi2 = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) ) );
  REQUIRE_THROWS( mi1->struct_union(mi2) );

  REQUIRE_NOTHROW( mi1 = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) ) );
  REQUIRE_NOTHROW( mi1->set_component
		   ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) ) ) );
  REQUIRE_NOTHROW( mi1->set_component
		   ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(30,40) ) ) );

  SECTION( "fit in dimension 1: multis are merged" ) {
    REQUIRE_NOTHROW( mi2->set_component
		     ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) ) ) );
    REQUIRE_NOTHROW( mi2->set_component
		     ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(31,41) ) ) );
  ...
  SECTION( "fit in dimension 2: multis are merged" ) {
    REQUIRE_NOTHROW( mi2->set_component
		     ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(12,22) ) ) );
    REQUIRE_NOTHROW( mi2->set_component
		     ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(30,40) ) ) );
    CHECK( mi1->can_union_in_place(mi2,diff) );
    CHECK( mi2->can_union_in_place(mi1,diff) );
  ...
  SECTION( "very unfit: case where union first/last are not in the originals" ) {
    ...
    REQUIRE_NOTHROW( mi2->set_component
  		     ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(20,30) ) ) );
    REQUIRE_NOTHROW( mi2->set_component
  		     ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(10,20) ) ) );
    REQUIRE_NOTHROW( mi3 = mi1->struct_union(mi2) );
    CHECK( mi3->is_multi() );
    auto first = mi3->first_index_r(), last = mi3->last_index_r();
    INFO( format("first = {}, last = {}",first.as_string(),last.as_string()) );
  ...
  SECTION( "union of more than two" ) {
    auto
      m1 = shared_ptr<multi_indexstruct>( new multi_indexstruct
      ( vector<shared_ptr<indexstruct>>
	{ shared_ptr<indexstruct>(new contiguous_indexstruct(1,1)),
	    shared_ptr<indexstruct>(new contiguous_indexstruct(1,1))
	    } ) ),
      m2 = shared_ptr<multi_indexstruct>( new multi_indexstruct
      ( vector<shared_ptr<indexstruct>>
	{ shared_ptr<indexstruct>(new contiguous_indexstruct(2,2)),
	    shared_ptr<indexstruct>(new contiguous_indexstruct(2,2))
	    } ) ),
      m3 = shared_ptr<multi_indexstruct>( new multi_indexstruct
      ( vector<shared_ptr<indexstruct>>
	{ shared_ptr<indexstruct>(new contiguous_indexstruct(3,3)),
	    shared_ptr<indexstruct>(new contiguous_indexstruct(3,3))
	    } ) );
    shared_ptr<multi_indexstruct> u;
    // m2 makes it a multi
    CHECK( !m1->contains(m2) );
  ...
  SECTION( "multi union with recombination" ) {
    auto 
      m1 = shared_ptr<multi_indexstruct>( new multi_indexstruct
      ( vector<shared_ptr<indexstruct>>
	{ shared_ptr<indexstruct>(new contiguous_indexstruct(1,2)),
	    shared_ptr<indexstruct>(new contiguous_indexstruct(1,1))
	    } ) ),
      m2 = shared_ptr<multi_indexstruct>( new multi_indexstruct
      ( vector<shared_ptr<indexstruct>>
    ...
    shared_ptr<multi_indexstruct> m3,u;
    REQUIRE_NOTHROW( u = m1->struct_union(m2) );
    INFO( format("union 1 & 2: {}",u->as_string()) );
    CHECK( u->multi_size()==2 );
    SECTION( "absorb" ) {
      REQUIRE_NOTHROW( m3 = shared_ptr<multi_indexstruct>( new multi_indexstruct
	( vector<shared_ptr<indexstruct>>
	  { shared_ptr<indexstruct>(new contiguous_indexstruct(2,2)),
	      shared_ptr<indexstruct>(new contiguous_indexstruct(1,1))
	      } ) ) );
...
TEST_CASE( "multidimensional intersection","[multi][indexstruct][114]" ) {
  shared_ptr<multi_indexstruct> mi1,mi2,mi3;
  REQUIRE_NOTHROW( mi1 = shared_ptr<multi_indexstruct>( new multi_indexstruct(1) ) );
  REQUIRE_NOTHROW( mi2 = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) ) );
  REQUIRE_THROWS( mi1->struct_union(mi2) );
  ...
  REQUIRE_NOTHROW( mi1 = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) ) );
  REQUIRE_NOTHROW( mi1->set_component
		   ( 0, shared_ptr<indexstruct>( new contiguous_indexstruct(0,10) ) ) );
  REQUIRE_NOTHROW( mi1->set_component
		   ( 1, shared_ptr<indexstruct>( new contiguous_indexstruct(0,10) ) ) );
...
TEST_CASE( "multi-dimensional minus","[multi][indexstruct][115]" ) {
  shared_ptr<multi_indexstruct> one,two,three;
  index_int lo = 1, hi = 4; bool success{true};
  REQUIRE_NOTHROW( one = shared_ptr<multi_indexstruct>( new multi_indexstruct
		   ( vector< shared_ptr<indexstruct> >
		     { shared_ptr<indexstruct>( new contiguous_indexstruct(1,2) ),
			 shared_ptr<indexstruct>( new contiguous_indexstruct(5,8) )
			 }
		     )
							     ) );
  SECTION( "royally fits" ) {
    REQUIRE_NOTHROW( two = shared_ptr<multi_indexstruct>( new multi_indexstruct
		     ( vector< shared_ptr<indexstruct> >
		       { shared_ptr<indexstruct>( new contiguous_indexstruct(0,3) ),
			   shared_ptr<indexstruct>( new contiguous_indexstruct(6,8) )
			   }
		       )
							       ) );
  }
  ...
  if (!success) { // this used to be unimplemented
    ...
    CHECK( three->get_component(1)->equals
	   ( shared_ptr<indexstruct>( new contiguous_indexstruct(5,5) ) ) );
  }
}

TEST_CASE( "find linear location","[multi][locate][indexstruct][116]" ) {
  shared_ptr<multi_indexstruct> outer,inner;
  index_int location;

  SECTION( "one-d" ) {
    outer = shared_ptr<multi_indexstruct>( new multi_indexstruct(1) );
    SECTION( "outer contiguous" ) {
      outer->set_component
	(0,shared_ptr<indexstruct>( new contiguous_indexstruct(11,20)) );
      SECTION( "same dimension" ) {
	inner = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) );
	inner->set_component
	  (0,shared_ptr<indexstruct>( new contiguous_indexstruct(15,16)) );
	inner->set_component
	  (1,shared_ptr<indexstruct>( new contiguous_indexstruct(8,9)) );
	REQUIRE_THROWS( location = outer->linear_location_of(inner) );
      }
      SECTION( "proper" ) {
	inner = shared_ptr<multi_indexstruct>( new multi_indexstruct(1) );
	inner->set_component
	  (0,shared_ptr<indexstruct>( new contiguous_indexstruct(15,16)) );
  ...
  SECTION( "two-d" ) {
    outer = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) );
    SECTION( "outer contiguous" ) {
      outer->set_component  // 10 deep
	(0,shared_ptr<indexstruct>( new contiguous_indexstruct(11,20)) );
      outer->set_component // 4 wide
	(1,shared_ptr<indexstruct>( new contiguous_indexstruct(5,8)) );
      SECTION( "same dimension" ) {
	inner = shared_ptr<multi_indexstruct>( new multi_indexstruct(1) );
	inner->set_component
	  (0,shared_ptr<indexstruct>( new contiguous_indexstruct(15,16)) );
	...
	inner->set_component
	  (0,shared_ptr<indexstruct>( new contiguous_indexstruct(15,16)) );
	inner->set_component
	  (1,shared_ptr<indexstruct>( new contiguous_indexstruct(6,8)) );
	REQUIRE_NOTHROW( location = outer->linear_location_of(inner) );
...
TEST_CASE( "relativize and find linear location","[multi][locate][indexstruct][117]" ) {
  // this uses the same structure as [112]
  shared_ptr<multi_indexstruct> outer,inner,relative;
  shared_ptr<indexstruct> relativec;
  index_int location;
...
TEST_CASE( "multi-dimensional operations","[multi][120]" ) {
  auto s1 = shared_ptr<multi_indexstruct>( new multi_indexstruct(2) );
  shared_ptr<multi_indexstruct> s2;
  shared_ptr<indexstruct> i0,i1;
  
  s1->set_component
    (0,shared_ptr<indexstruct>(  new contiguous_indexstruct(10,20) ) );
  s1->set_component
    (1,shared_ptr<indexstruct>(  new strided_indexstruct(25,50,5) ) );
...
TEST_CASE( "multi dimensional iteration, 1d","[multi][range][125]" ) {
  ...
  multi_indexstruct segment
    ( shared_ptr<indexstruct>( new contiguous_indexstruct(f,l) ) );
...
TEST_CASE( "operate multi indexstruct","[multi][operate][132]" ) {
  shared_ptr<multi_indexstruct> brick,sh_brick;

  REQUIRE_NOTHROW( brick = shared_ptr<multi_indexstruct>( new multi_indexstruct(3) ) );
  brick->set_component
    ( 0,shared_ptr<indexstruct>(  new contiguous_indexstruct(10,20) ) );
  brick->set_component
    ( 1,shared_ptr<indexstruct>(  new strided_indexstruct(25,50,5) ) );
  brick->set_component
    ( 2,shared_ptr<indexstruct>(  new strided_indexstruct(5,7,2) ) );
...
TEST_CASE( "Bilinear shift test","[operate][multi][133]" ) {
  ...
  shared_ptr<multi_indexstruct> gamma_struct,
    halo_struct = shared_ptr<multi_indexstruct>( new empty_multi_indexstruct(dim) ),
    truncation = shared_ptr<multi_indexstruct>( new contiguous_multi_indexstruct
    ( domain_coordinate( vector<index_int>{0,0} ),
      domain_coordinate( vector<index_int>{11,11} ) ) );

  gamma_struct = shared_ptr<multi_indexstruct>( new contiguous_multi_indexstruct
    ( domain_coordinate( vector<index_int>{0,0} ),
...
TEST_CASE( "multi sigma operators","[multi][sigma][140]" ) {
  ...
  SECTION( "from coordinate-to-struct operator" ) { path="coord-to-struct";
	...
	(dim,[] (const domain_coordinate &c) -> shared_ptr<multi_indexstruct> {
	  int dim = c.get_dimensionality();
	  //print("Apply multi sigma in d={}\n",dim);
	  auto m = shared_ptr<multi_indexstruct>( new multi_indexstruct(dim) );
	  for (int id=0; id<dim; id++) {
	    auto shift_struct = shared_ptr<indexstruct>
  ...
  shared_ptr<multi_indexstruct> block,blocked;
  ...
  REQUIRE_NOTHROW( block = shared_ptr<multi_indexstruct>( new contiguous_multi_indexstruct(first,last) ) );
