...
int indexstruct::type_as_int() const {
...
bool indexstruct::equals( std::shared_ptr<indexstruct> idx ) const {
...
shared_ptr<indexstruct> indexstruct::operate
...
bool indexstruct::contains_element_in_range(index_int idx) const {
  return idx>=first_index() && idx<=last_index();
};

//! Base test for disjointness; derived classes will build on this.
bool indexstruct::disjoint( shared_ptr<indexstruct> idx ) {
  return first_index()>idx->last_index() || last_index()<idx->first_index();
};

shared_ptr<indexstruct> indexstruct::operate( const sigma_operator &op, index_int lo,index_int hi ) const {
  shared_ptr<indexstruct>
    noleft = this->operate(op)->truncate_left(lo),
    noright = noleft->truncate_right(hi);
  return noright;
...
shared_ptr<indexstruct> indexstruct::operate
    ( const ioperator &op,shared_ptr<indexstruct> outer ) const {
...
//! Operate, then cut boundaries to fit within `outer'.
shared_ptr<indexstruct> indexstruct::operate
    ( const sigma_operator &op,shared_ptr<indexstruct> outer ) const {
  index_int lo = outer->first_index(), hi = outer->last_index();
  return this->operate(op,lo,hi);
};

//! Let `trunc' be the first index in the truncated struct.
shared_ptr<indexstruct> indexstruct::truncate_left( index_int trunc ) {
  if (trunc<=first_index()) {
    return this->make_clone();
    //return shared_ptr<indexstruct>( this->make_clone() ); //shared_from_this();
  } else {
...
shared_ptr<indexstruct> indexstruct::truncate_right( index_int trunc ) {
  if (trunc>=last_index()) {
 ...
 **** Empty indexstruct
 ****/
shared_ptr<indexstruct> empty_indexstruct::add_element( const index_int idx ) {
  //  return shared_ptr<indexstruct>( new indexed_indexstruct(1,&idx) );
  return shared_ptr<indexstruct>( make_shared<indexed_indexstruct>(1,&idx) );
};

shared_ptr<indexstruct> empty_indexstruct::struct_union
...
bool strided_indexstruct::equals( shared_ptr<indexstruct> idx ) const {
  if (idx->is_empty())
    return false;
...
shared_ptr<indexstruct> strided_indexstruct::add_element( const index_int idx ) {
  if (contains_element(idx))
    ...
    auto indexed = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>(this) );
    return indexed->add_element(idx);
  }
};

bool strided_indexstruct::contains_element( index_int idx ) const {
  return idx>=first && idx<=last && (idx-first)%stride_amount==0;
};

index_int strided_indexstruct::find( index_int idx ) const {
  if (!contains_element(idx))
...
bool strided_indexstruct::contains( shared_ptr<indexstruct> idx ) const {
...
bool strided_indexstruct::disjoint( shared_ptr<indexstruct> idx ) {
...
bool strided_indexstruct::has_intersect( shared_ptr<indexstruct> idx ) {
  if (idx->is_empty())
    return false;
  if (contains(idx))
    return true;
...
shared_ptr<indexstruct> strided_indexstruct::translate_by( index_int shift ) {
  return shared_ptr<indexstruct>{
    new strided_indexstruct(first+shift,last+shift,stride_amount) };
};

shared_ptr<indexstruct> strided_indexstruct::intersect
    ( shared_ptr<indexstruct> idx ) {
  if (idx->is_empty())
    return shared_ptr<indexstruct>( make_shared<empty_indexstruct>() );
  if (contains(idx))
  ...
  if (strided!=nullptr) { // strided & strided
	...
	if (mn>mx)
	  ...
	  return shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(mn,mx) );
	else
	  return shared_ptr<indexstruct>
      ...
      auto rstruct = shared_ptr<indexstruct>( make_shared<empty_indexstruct>() );
      for ( auto i : *this ) {
...
shared_ptr<indexstruct> strided_indexstruct::minus
    ( shared_ptr<indexstruct> idx ) const {
  // easy cases
  if (idx->is_empty())
    return this->make_clone();
  //return shared_ptr<indexstruct>( this->make_clone());
  ...
  if (strided!=nullptr) { // strided & strided
    if (stride_amount==strided->stride_amount) {
      shared_ptr<indexstruct> stridmin{nullptr};
      if (first%stride_amount!=strided->first%strided->stride_amount) {
	...
	if (stride_amount==1)
	  stridmin = shared_ptr<indexstruct>{ make_shared<contiguous_indexstruct>(f,l) };
	else
	  stridmin = shared_ptr<indexstruct>{ make_shared<strided_indexstruct>(f,l,stride_amount) };
      }
      return stridmin;
    } else {
      auto
  ...
  if (indexed!=nullptr) { // strided & indexed
    ...
    return this_index->minus(idx); //(shared_ptr<indexstruct>(indexed));
  }
...
shared_ptr<indexstruct> strided_indexstruct::relativize_to
    ( shared_ptr<indexstruct> idx,bool trace ) {
  if (trace)
  ...
  if (strided!=nullptr) { // strided & strided
    if (stride_amount==strided->stride_amount) { // same stride
	  ...
	  return shared_ptr<indexstruct>{ make_shared<empty_indexstruct>() };
          ...
          return shared_ptr<indexstruct>{ make_shared<contiguous_indexstruct
  ...
  indexed_indexstruct *indexed = dynamic_cast<indexed_indexstruct*>(idx.get());
  if (indexed!=nullptr) {
    auto relext = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
    for (auto i : *this) {
  ...
  if (composite!=nullptr) {
    ...
    // general case
    auto rel = shared_ptr<indexstruct>( make_shared<empty_indexstruct>() );
    for ( auto s : structs ) {
...
shared_ptr<indexstruct> strided_indexstruct::convert_to_indexed() const {
  auto indexed = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
...
shared_ptr<indexstruct> strided_indexstruct::struct_union
    ( shared_ptr<indexstruct> idx ) {
  if (contains(idx))
  ...
  if (strided!=nullptr) {
	...
	return shared_ptr<indexstruct>{make_shared<strided_indexstruct>(mn,mx,stride_amount)};
  ...
  if (indexed!=nullptr) {
    ...
    return shared_ptr<indexstruct>( extstrided->force_simplify() );
  }
...
shared_ptr<indexstruct> strided_indexstruct::split
    ( shared_ptr<indexstruct> idx ) {
  int s = stride();
  auto res = make_shared<composite_indexstruct>();
  auto f = first_index(), ff = idx->first_index(), l = last_index(), ll = idx->last_index();
  shared_ptr<indexstruct> s1,s2;
  if (contains_element_in_range(ff)) {
    s1 = shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(f,ff-1));
    s2 = shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(ff,l));
  } else if (contains_element_in_range(ll)) {
    s1 = shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(f,ll));
    s2 = shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(ll+1,l));
...
*/
shared_ptr<indexstruct> indexed_indexstruct::add_element( const index_int idx ) {
...
shared_ptr<indexstruct> indexed_indexstruct::translate_by( index_int shift ) {
  auto translated = shared_ptr<indexstruct>{
...
shared_ptr<indexstruct> indexed_indexstruct::simplify() {
...
*/
shared_ptr<indexstruct> indexed_indexstruct::force_simplify() const {
  try {
    if (0) {
      ...
      return
	shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(first_index(),last_index()) );
    } else {
	...
	if (stride==1)
	  return shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(first,last) );
	else
	  return shared_ptr<indexstruct>( make_shared<strided_indexstruct>(first,last,stride) );
      } else {
	...
	for (int find_left=ileft; find_left<iright; find_left++) {
	  ...
	  for (int find_right=top_right; find_right>find_left+1; find_right--) {
		...
		auto left_part = this->minus
		  ( shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(left,last)) );
		//print("composing with left: {}\n",left_part->as_string());
		comp->push_back(left_part);
		//print(" .. gives {}\n",comp->as_string());			 
	      } 
	      shared_ptr<indexstruct> strided;
	      if (stride==1)
		strided = shared_ptr<indexstruct>
		...
		auto right_part = this->minus
		  ( shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(first,right)) );
...
bool indexed_indexstruct::is_strided_between_indices(int ileft,int iright,int &stride) const {
  index_int first = get_ith_element(ileft), last = get_ith_element(iright),
...
shared_ptr<indexstruct> indexed_indexstruct::intersect
    ( shared_ptr<indexstruct> idx ) {
  /*
  ...
  if (strided!=nullptr) { // indexed & strided
    auto limited = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
...
bool indexed_indexstruct::contains( shared_ptr<indexstruct> idx ) const {
  if (idx->local_size()==0) return true;
  if (this->local_size()==0) return false;
...
//! \todo weird clone because of a const problem.
shared_ptr<indexstruct> indexed_indexstruct::minus
    ( shared_ptr<indexstruct> idx ) const{
  if (idx->is_empty())
    return this->make_clone();
  if (idx->contains(this->make_clone())) //(this->shared_from_this()))
  ...
  strided_indexstruct *strided = dynamic_cast<strided_indexstruct*>(idx.get());
  if (strided!=nullptr) {
    auto indexminus = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
    for (auto v : indices)
...
bool indexed_indexstruct::disjoint( shared_ptr<indexstruct> idx ) {
...
bool indexed_indexstruct::equals( shared_ptr<indexstruct> idx ) const {
  auto nonconst = make_clone();
  return nonconst->contains(idx) && idx->contains(nonconst);
};

shared_ptr<indexstruct> indexed_indexstruct::relativize_to
    ( shared_ptr<indexstruct> idx,bool trace ) {
  if (!idx->contains(this->shared_from_this()))
    throw(fmt::format("Need containment for relativize {} to {}",
		      this->as_string(),idx->as_string()));
  ...
  if (strided!=nullptr) {
    ...
    return shared_ptr<indexstruct>{ this->translate_by( -shift ) };
  }
  ...
  indexed_indexstruct *indexed = dynamic_cast<indexed_indexstruct*>(idx.get());
  if (indexed!=nullptr) {
    auto relative = shared_ptr<indexstruct>{ make_shared<indexed_indexstruct>() };
    int count = 0;
    for (auto v : indices)
...
shared_ptr<indexstruct> indexed_indexstruct::struct_union
    ( shared_ptr<indexstruct> idx ) {
  if (contains(idx)) {
    return this->make_clone();
  } else if (idx->contains(this->shared_from_this())) {
...
*/
void composite_indexstruct::push_back( shared_ptr<indexstruct> idx ) {
...
shared_ptr<indexstruct> composite_indexstruct::make_clone() const {
...
*/
bool composite_indexstruct::contains( shared_ptr<indexstruct> idx ) const {
  /*
   * Case: contains strided. 
...
bool composite_indexstruct::contains_element( index_int idx ) const {
  //for (auto s : structs)
  for (int is=0; is<structs.size(); is++) {
...
index_int composite_indexstruct::find( index_int idx ) const {
  index_int
...
bool composite_indexstruct::disjoint( shared_ptr<indexstruct> idx ) {
 ...
 */
shared_ptr<indexstruct> composite_indexstruct::struct_union
    ( shared_ptr<indexstruct> idx) {
...
shared_ptr<indexstruct> composite_indexstruct::intersect
    ( shared_ptr<indexstruct> idx ) {
  ...
  auto result = shared_ptr<indexstruct>{ make_shared<empty_indexstruct>() };
  for (auto s : structs) {
...
shared_ptr<indexstruct> composite_indexstruct::minus
    ( shared_ptr<indexstruct> idx ) const {
...
shared_ptr<indexstruct> composite_indexstruct::relativize_to
    ( shared_ptr<indexstruct> idx,bool trace ) {
  auto rel = shared_ptr<indexstruct>( make_shared<empty_indexstruct>() );
  for ( auto s : structs ) {
    try {
...
shared_ptr<indexstruct> composite_indexstruct::force_simplify() const {
  ...
  try {
	...
	if (can_merge) {
	  ...
	  for (int is=0; is<ns; is++) {
		...
		auto i_struct = shared_ptr<indexstruct>(make_shared<contiguous_indexstruct>(i));
	...
	for (int js=0; js<structs.size(); js++) { // loop over all others
	  ...
	  if (jstruct->is_contiguous()) {
		...
		( shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(first) ) );
	  ...
	  if (jstruct->is_contiguous()) {
		...
		jstruct = jstruct->add_element(last);
	      istruct = istruct->minus
		( shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(last) ) );
	      left = !istruct->is_empty();
	      if (left)
 ...
 */
shared_ptr<indexstruct> composite_indexstruct::over_simplify() {
  { /*
    ...
    if (gap) {
      ...
      auto new_s = shared_ptr<indexstruct>
...
shared_ptr<indexstruct> composite_indexstruct::convert_to_indexed() const {
  auto idx = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
  for ( auto s : structs ) {
...
bool composite_indexstruct::equals( shared_ptr<indexstruct> idx ) const {
  if (structs.size()>1)
    return false;
  else
...
void indexstructure::push_back( contiguous_indexstruct &&idx ) {
  ...
  if (comp!=nullptr) {
    auto cnt_ptr = std::shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(idx) );
...
shared_ptr<indexstruct> strided_indexstruct::operate( const ioperator &op ) const {
  shared_ptr<indexstruct> operated{nullptr};
  if (op.is_none_op()) {
    return this->make_clone();
    ...
    if (stride()==1)
      operated = shared_ptr<indexstruct>{ make_shared<contiguous_indexstruct>(f,l) };
    else
      operated = shared_ptr<indexstruct>{ make_shared<strided_indexstruct>(f,l,stride()) };
    ...
    if (new_stride==1)
      operated =
	shared_ptr<indexstruct>{ make_shared<contiguous_indexstruct>(new_first,new_last) };
    else
      operated =
	shared_ptr<indexstruct>{ make_shared<strided_indexstruct>(new_first,new_last,new_stride) };
  } else if (op.is_function_op()) {
    auto rstruct = shared_ptr<indexstruct>{ make_shared<indexed_indexstruct>() };
...
*/
shared_ptr<indexstruct> strided_indexstruct::operate( const sigma_operator &op ) const {
...
shared_ptr<indexstruct> indexed_indexstruct::operate( const ioperator &op ) const {
  auto shifted = shared_ptr<indexstruct>( make_shared<indexed_indexstruct>() );
...
shared_ptr<indexstruct> composite_indexstruct::operate( const ioperator &op ) const {
  auto rstruct = shared_ptr<composite_indexstruct>{ make_shared<composite_indexstruct>() };
  for (auto s : structs)
...
multi_indexstruct::multi_indexstruct( int d ) {
  ...
  for (int dm=0; dm<d; dm++)
    components.push_back( shared_ptr<indexstruct>( make_shared<empty_indexstruct>() ) );
...
multi_indexstruct::multi_indexstruct( domain_coordinate c )
  ...
  for (int id=0; id<dim; id++)
    set_component(id,shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(c[id])) );
};
...
multi_indexstruct::multi_indexstruct( domain_coordinate f,domain_coordinate l )
  ...
  for (int id=0; id<dim; id++)
    set_component(id,shared_ptr<indexstruct>( make_shared<contiguous_indexstruct>(f[id],l[id]) ));
};

//! Constructor by wrapping up a single \ref indexstruct as the only dimension
multi_indexstruct::multi_indexstruct( shared_ptr<indexstruct> d1struct )
  : multi_indexstruct(1) {
  set_component(0,d1struct);
};

//! Constructor from individual \ref indexstruct objects. \todo should we clone?
multi_indexstruct::multi_indexstruct( std::vector< shared_ptr<indexstruct> > structs )
...
multi_indexstruct::multi_indexstruct( std::vector<index_int> sizes )
...
int multi_indexstruct::type_as_int() const {
  int type{-1};
  int d = get_dimensionality();
  for (int id=0; id<d; id++) {
    if (type<0)
...
*/
bool multi_indexstruct::is_strided() const {
  int d = get_dimensionality();
  for (int id=0; id<d; id++)
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::make_clone() const {
  ...
  if (is_multi()) {
    ...
    for (int id=0; id<dim; id++) {
      shared_ptr<indexstruct> idx = get_component(id), nidx;
      if (idx==nullptr)
	nidx = idx;
      else
	nidx = idx->make_clone();
...
*/
void multi_indexstruct::set_component( int d, shared_ptr<indexstruct> cmp ) {
  if (multi.size()>0)
...
shared_ptr<indexstruct> multi_indexstruct::get_component(int d) const {
  if (multi.size()>0) {
...
index_int multi_indexstruct::volume() const {
  if (multi.size()>0) {
...
const domain_coordinate &multi_indexstruct::local_size_r() const {
...
*/
const domain_coordinate &multi_indexstruct::first_index_r() const {
...
/*! Return a vector of last indices
  ...
  not make a copy or otherwise remember it.
*/
const domain_coordinate &multi_indexstruct::last_index_r() const {
...
std::vector<domain_coordinate> multi_indexstruct::get_corners() const {
  int dim = get_dimensionality();
...
const multi_indexstruct &multi_indexstruct::enclosing_structure_r() const {
...
const shared_ptr<multi_indexstruct> multi_indexstruct::enclosing_structure() const {
  if (stored_enclosing_structure==nullptr)
    stored_enclosing_structure =
      shared_ptr<multi_indexstruct>
      ( make_shared<contiguous_multi_indexstruct>( first_index_r(),last_index_r() ) );
...
*/
domain_coordinate *multi_indexstruct::location_of
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::relativize_to
...
*/
index_int multi_indexstruct::linear_location_of( shared_ptr<multi_indexstruct> idx ) const {
  return idx->first_index_r().linear_location_in( this->first_index_r(),this->last_index_r() );
};

index_int multi_indexstruct::linear_location_in( shared_ptr<multi_indexstruct> idx ) const {
...
index_int multi_indexstruct::linear_location_in( const multi_indexstruct &idx ) const {
  auto find = first_index_r(),
    first = idx.first_index_r(), last = idx.last_index_r();
  index_int s = find.linear_location_in(first,last);
  return s;
...
domain_coordinate multi_indexstruct::linear_offsets(shared_ptr<multi_indexstruct> inner) const {
  int dim = get_same_dimensionality(inner->get_dimensionality());
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::operate( const ioperator &op ) const {
  int dim = get_dimensionality();
  auto rstruct = make_shared<multi_indexstruct>(dim) ;
  for (int id=0; id<dim; id++) {
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::operate
...
shared_ptr<multi_indexstruct> multi_indexstruct::operate( multi_ioperator *op ) const {
...
shared_ptr<multi_indexstruct> multi_indexstruct::operate( const multi_sigma_operator &op ) const {
 ...
 */
shared_ptr<multi_indexstruct> multi_indexstruct::struct_union
    (shared_ptr<multi_indexstruct> other) {
  int dim = get_same_dimensionality(other->get_dimensionality());
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::struct_union_in_place
...
shared_ptr<multi_indexstruct> multi_indexstruct::split_along_dim
    (int splitdim,shared_ptr<indexstruct> induce) const {
  int dim = get_dimensionality();
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::force_simplify(bool trace) const {
  try {
    if (is_multi()) {
...
*/
shared_ptr<multi_indexstruct> multi_indexstruct::intersect
    ( shared_ptr<multi_indexstruct> other ) {
...
shared_ptr<multi_indexstruct> multi_indexstruct::intersect
  ...
  for (int id=0; id<dim; id++)
    rstruct->set_component
      (id,shared_ptr<indexstruct>
       ( get_component(id)->intersect(other.get_component(id))) );
  return rstruct;
};

bool multi_indexstruct::contains( shared_ptr<multi_indexstruct> other ) const {
  return contains( *(other.get()) );
};

//! Multi_indexstruct containment requires all dimensions to contain
bool multi_indexstruct::contains( const multi_indexstruct &other ) const {
  bool trace{false};
  int dim  = get_same_dimensionality( other.get_dimensionality() );
  if (trace)
...
//! \todo weird clone because of a const problem.
shared_ptr<multi_indexstruct> multi_indexstruct::minus
...
bool multi_indexstruct::contains_element( const domain_coordinate &i ) const {
  if (is_multi()) {
    for ( auto s : multi )
...
bool multi_indexstruct::equals( shared_ptr<multi_indexstruct> other ) const {
  if (is_multi() || other->is_multi()) {
...
bool multi_indexstruct::operator==( const multi_indexstruct &other ) const {
  if (is_multi() || other.is_multi()) {
    throw(std::string("multi multi equals not implemented"));
  } else {
    for (int d=0; d<get_dimensionality(); d++)
...
index_int domain_coordinate::volume() const {
  index_int s{1};
...
bool domain_coordinate::operator==( const domain_coordinate &&other ) const {
  //print("eq test {} vs {}\n",as_string(),other.as_string());
  int dim = get_dimensionality();
...
bool domain_coordinate::operator==( const domain_coordinate &other ) const {
  ...
  return operator==(move(other_copy));
  //print("eq test {} vs {}\n",as_string(),other.as_string());
  int dim = get_dimensionality();
  if (dim!=other.get_dimensionality()) return false;
  for (int id=0; id<dim; id++)
...
bool domain_coordinate::is_on_left_face
    (int d,shared_ptr<multi_indexstruct> enclosing) const {
...
index_int domain_coordinate::linear_location_in
    ( shared_ptr<multi_indexstruct> bigstruct ) const {
  return linear_location_in( *(bigstruct.get()) );
};
index_int domain_coordinate::linear_location_in( const multi_indexstruct &bigstruct ) const {
...
shared_ptr<indexstruct> sigma_operator::operate(index_int i) const {
  if (lambda_s)
    throw(std::string("Can not operate on point: only defined for structs"));
  if (is_point_operator()) {
    return shared_ptr<indexstruct>{make_shared<contiguous_indexstruct>( point_func.operate(i) )};
  } else {
    return func(i);
  }
};
...
*/
shared_ptr<indexstruct> sigma_operator::operate( shared_ptr<indexstruct> i) const {
  if (lambda_s) {
    try {
      return sfunc(*i);
    } catch (std::string c) {
...
shared_ptr<multi_indexstruct> multi_sigma_operator::operate
  ...
  if (0) {
	...
	newstruct = shared_ptr<indexstruct>( oldstruct->operate( operators[id] ) );
