...
int main(int argc,char **argv) {
  ...
  try {
    ...
    // two-d decomposition
    int laplace_dim = 2;
    const auto layout = arch->get_proc_layout(laplace_dim);
    decomposition decomp = IMP_decomposition(arch);
#if defined(IMPisMPI) || defined(IMPisPRODUCT)
    const auto mycoord = decomp.coordinate_from_linear(mytid);
#endif
    ...
    auto nodes_dist = shared_ptr<distribution>
    ...
    vector<shared_ptr<object>> sequence(nsteps);
    for (int step=0; step<nsteps; step++) {
      //sequence.at(step) = std::shared_ptr<IMP_object>( new IMP_object(nodes_dist) );
      sequence.at(step) = shared_ptr<object>( new IMP_object(nodes_dist) );
    }

    /* Create algorithm, and initialize the sequences */
    auto grid_evolve = shared_ptr<algorithm>( new IMP_algorithm(decomp) );
    grid_evolve->add_kernel
      ( shared_ptr<kernel>( new IMP_setconstant_kernel( sequence.at(0) ),1.) );
    if (blockd)
      grid_evolve->add_kernel
	( shared_ptr<kernel>( new IMP_setconstant_kernel( sequence.at(1) ),1.) );
	...
	in1 = 2*step; out1 = 2*step+1; in2 = 2*step+1; out2 = 2*step+2;
      ...
      //k = shared_ptr<kernel>( new IMP_kernel( sequence.at(in1) ).get(),sequence.at(out1).get() );
      k = shared_ptr<kernel>( new IMP_kernel( sequence.at(in1) ),sequence.at(out1) );
      k->add_sigma_stencil(bilinear_stencil);
      k->set_localexecutefn( &laplace_bilinear_fn );
      grid_evolve->add_kernel(k);

      //k = shared_ptr<kernel>( new IMP_kernel( sequence.at(in2) ).get(),sequence.at(out2).get() );
      k = shared_ptr<kernel>( new IMP_kernel( sequence.at(in2) ),sequence.at(out2) );
      k->add_sigma_stencil(bilinear_stencil);
      k->set_localexecutefn( &laplace_bilinear_fn );
      grid_evolve->add_kernel(k);
    }
