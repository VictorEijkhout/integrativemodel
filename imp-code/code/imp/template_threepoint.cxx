...
int main(int argc,char **argv) {
  ...
  */
  vector<shared_ptr<object>> all_objects(nsteps+1);
   ...
   */
  auto blocked = shared_ptr<distribution>( new IMP_block_distribution(env,globalsize) );
  ...
  */
  {
    ...
    int step = 0;
    shared_ptr<object> gen_object = shared_ptr<object>( new IMP_object( blocked ) );
    all_objects.at(step) = gen_object;
    auto gen_step = shared_ptr<kernel>( new IMP_kernel(gen_object) );
  ...
  for (int step=1; step<=nsteps; ++step) {
    ...
    auto
      output_vector = shared_ptr<object>( new IMP_object( blocked ) ),
      input_vector = all_objects.at(step-1);
    all_objects.at(step) = output_vector;
    auto update_step = 
      shared_ptr<kernel>( new IMP_kernel(input_vector,output_vector) );
    update_step->set_localexecutefn( &threepoint_execute );
    update_step->add_sigma_oper( ioperator(">>1") );
    update_step->add_sigma_oper( ioperator("<<1") );
    update_step->add_sigma_oper( ioperator("none") );
...
// #endif
  ...
  if (env->dodebug() & DEBUG_VECTORS) {
    ...
    all_objects.at(0)->print();
    env->print("Final result:\n");
    all_objects.at(nsteps)->print();
  }
