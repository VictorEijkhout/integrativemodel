...
int main(int argc,char **argv) {
  ...
  try {
    ...
    auto
      block_dist = shared_ptr<distribution>( new IMP_block_distribution(decomp,nglobal) );
    auto global_structure = block_dist->get_global_structure();
    // in between we have non-disjoint distributions
    vector<shared_ptr<distribution> halo_dists(nsteps);
	...
	out_dist = block_dist;
      ...
      if (blockd>0)
	halo_dists[istep] = shared_ptr<distribution
    ...
    vector<shared_ptr<object>> objects(nsteps+1);
    objects[0] = shared_ptr<object>( new IMP_object(block_dist) );
    objects[nsteps] = shared_ptr<object>( new IMP_object(block_dist) );
    for (int istep=1; istep<nsteps; istep++)
      objects[istep] = shared_ptr<object>( new IMP_object(halo_dists[istep]) );

    auto sstep = shared_ptr<algorithm>( new IMP_algorithm(decomp) );
    sstep->add_kernel( shared_ptr<kernel>( new IMP_origin_kernel(objects[0]) ) );
    vector<shared_ptr<kernel>> kernels(nsteps);
    for (int istep=0; istep<nsteps; istep++) {
      auto k = shared_ptr<kernel>( new IMP_kernel(objects[istep],objects[istep+1]) );
    ...
    auto copy_back = shared_ptr<algorithm>( new IMP_algorithm(decomp) );
    copy_back->add_kernel( shared_ptr<kernel>( new IMP_origin_kernel(objects[nsteps]) ) );
    copy_back->add_kernel( shared_ptr<kernel>( new IMP_copy_kernel(objects[nsteps],objects[0]) ) );
