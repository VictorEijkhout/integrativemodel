...
TEST_CASE( "Analyze gather dependencies","[collective][dependence][10]") {
  ...
  REQUIRE_NOTHROW( alpha = shared_ptr<distribution>( new product_block_distribution(decomp,product_nprocs) ) );
  index_int av;
  ...
  shared_ptr<distribution> gamma;
  REQUIRE_NOTHROW( gamma = shared_ptr<distribution>( new product_gathered_distribution(decomp) ) );
  ...
  {
    shared_ptr<distribution> omp_gamma;
    REQUIRE_NOTHROW
  ...
  CHECK_NOTHROW( gather_task = dynamic_cast<product_task*>(gather->get_tasks().at(0)) );
  {
    ...
    auto d = gather->get_beta_distribution();
...
TEST_CASE( "Analyze gather dependencies in one go","[collective][dependence][11]") {
  ...
  // alpha distribution is one scalar per processor
  shared_ptr<distribution> alpha;
  REQUIRE_NOTHROW( alpha = shared_ptr<distribution>( new product_block_distribution(decomp,-1,product_nprocs) ) );
  // gamma distribution is gathering those scalars
  shared_ptr<distribution> gamma = shared_ptr<distribution>( new product_gathered_distribution(decomp) );
  // general stuff
  product_object
...
TEST_CASE( "Actually gather something","[collective][dependence][12]") {
  ...
  // gamma distribution is gathering those scalars
  // 
  auto gamma = shared_ptr<distribution>( new product_gathered_distribution(decomp) );
  ...
  { // investigate the OpenMP stuff
	...
	shared_ptr<distribution> indist,halodist;
  ...
  }
  {
    ...
    shared_ptr<distribution> product_distr; shared_ptr<distribution> pre_product_distr;
...
TEST_CASE( "Gather more than one something","[collective][dependence][13]") {
  ...
  // k scalars per proc
  auto alpha = shared_ptr<distribution>( new product_block_distribution(decomp,k,1,-1) );
  ...
  // gamma distribution is gathering those scalars
  auto gamma = shared_ptr<distribution>( new product_gathered_distribution(decomp,k) );
...
TEST_CASE( "Gather and sum as two kernels","[collective][dependence][14]") {
  ...
  auto distributed = shared_ptr<distribution>( new product_block_distribution(decomp,product_nprocs) );
  ...
  auto collected = shared_ptr<distribution>( new product_gathered_distribution(decomp) );
  product_object       *gathered  = new product_object(collected);

  // final distribution is redundant one scalar
  auto replicated     = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
...
TEST_CASE( "Inner product in three kernels","[collective][15]" ) {
  ...
  auto
    alpha = shared_ptr<distribution>( new product_block_distribution(decomp,10+2*mytid,-1) ),
    local_scalar = shared_ptr<distribution>( new product_block_distribution(decomp,1,-1) ),
    gathered_scalar = shared_ptr<distribution>( new product_gathered_distribution(decomp) ),
    summed_scalar = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
