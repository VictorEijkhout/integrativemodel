...
TEST_CASE( "Copy kernel","[kernel][copy][31]" ) {
  ...
  auto last_coordinate = domain_coordinate( vector<index_int>{nlocal-1} );
  auto blocked = shared_ptr<distribution>( new product_block_distribution(decomp,gsize) );
  auto
    x = shared_ptr<object>( new product_object(blocked) ),
    xnew = shared_ptr<object>( new product_object(blocked) ),
    z = shared_ptr<object>( new product_object(blocked) ),
    r = shared_ptr<object>( new product_object(blocked) ), 
    rnew = shared_ptr<object>( new product_object(blocked) ),
    p = shared_ptr<object>( new product_object(blocked) ), 
    pold = shared_ptr<object>( new product_object(blocked) ),
    q = shared_ptr<object>( new product_object(blocked) ), 
    qold = shared_ptr<object>( new product_object(blocked) );
  
  // scalars, all redundantly replicated
  auto scalar = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
  auto
    rr  = shared_ptr<object>( new product_object(scalar) ), 
    rrp = shared_ptr<object>( new product_object(scalar) ),
    rnorm = shared_ptr<object>( new product_object(scalar) ),
    pap = shared_ptr<object>( new product_object(scalar) ),
    alpha = shared_ptr<object>( new product_object(scalar) ), 
    beta = shared_ptr<object>( new product_object(scalar) );
  
  kernel *rrcopy;
  int n; const char *path;
  decltype( rr->get_data(mycoord) ) rdata;
  decltype( rrp->get_data(mycoord) ) sdata;
...
TEST_CASE( "Scale kernel","[task][kernel][execute][33]" ) {
  ...
  decltype( yvector->get_data(mycoord) ) halo_data,ydata;
  ...
  SECTION( "scale by constant" ) {
    ...
    SECTION( "kernel as kernel from replicated object" ) {
      ...
      double xval = 2;
      auto scalar = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
      auto x = shared_ptr<object>( new product_object(scalar) );
      x->set_value(xval);
...
TEST_CASE( "Beta from sparse matrix","[beta][sparse][41]" ) {
  ...
  int nlocal=20,gsize=nlocal*ntids;
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal,-1) );
...
TEST_CASE( "Actual sparse matrix","[beta][sparse][spmvp][42]" ) {
  ...
  for (vector<message*>::iterator m=rmsgs->begin(); m!=rmsgs->end(); ++m) {
    message *msg = (message*)(*m);
...
TEST_CASE( "matrix kernel analysis","[kernel][spmvp][44]" ) {
  ...
  auto blocked =
    shared_ptr<distribution>( new product_block_distribution(decomp,g) );
  auto
    x = shared_ptr<object>( new product_object(blocked) ), 
    y = shared_ptr<object>( new product_object(blocked) );
...
TEST_CASE( "matrix iteration shift left","[kernel][45]" ) {
  ...
  auto blocked =
    shared_ptr<distribution>( new product_block_distribution(decomp,g) );
  auto x = shared_ptr<object>( new product_object(blocked) );
  ...
  auto y = vector<shared_ptr<object>>(nsteps);
  for (int i=0; i<nsteps; i++) {
    y[i] = shared_ptr<object>( new product_object(blocked) );
  }
  ...
  // make a queue
  auto queue = shared_ptr<algorithm>( new product_algorithm(arch) );
  ...
  for (auto t : tasks ) { //=tasks->begin(); t!=tasks->end(); ++t) {
    if (t.get_step()==x->get_object_number()) {
      ...
      vector<dependency> deps = t.get_dependencies();
      CHECK( deps.size()==1 );
      auto dep = deps.at(0);

      INFO( "step " << (*t)->get_step() );

      vector<message*> recv,send;
...
TEST_CASE( "compound kernel queue","[kernel][queue][50]" ) {
  ...
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto
    block = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal*ntids) ),
    scalar = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
  auto
    x = shared_ptr<object>( new product_object(block) ),
    y = shared_ptr<object>( new product_object(block) ),
    xy = shared_ptr<object>( new product_object(scalar) );
  kernel
  ...
  for ( t : tsks ) { //vector<task*>::iterator t=tsks->begin(); t!=tsks->end(); ++t) {
    int ik;
...
TEST_CASE( "r_norm1, different ways","[kernel][collective][55]" ) {
  ...
  double *data;
  auto local_scalar = shared_ptr<distribution>( new product_block_distribution(decomp,1,-1) );
  auto local_value = shared_ptr<object>( new product_object(local_scalar) );
  REQUIRE_NOTHROW( data = local_value->get_data(mycoord) );
  data[0] = (double)mytid;

  shared_ptr<distribution> replicated;
  REQUIRE_NOTHROW( replicated = shared_ptr<distribution>( new product_replicated_distribution(decomp) ) );
  auto global_sum;
  REQUIRE_NOTHROW( global_sum = shared_ptr<object>( new product_object(replicated) ) );
  ...
  SECTION( "spell out the way to the top" ) {
	...
	REQUIRE_NOTHROW( groups->set_processor_structure(p,pstruct ) );
      ...
      REQUIRE_NOTHROW( locally_grouped = shared_ptr<distribution>( new product_distribution(groups) ) );
    }
	...
	*partials = new parallel_indexstruct( env->get_architecture() );
	...
	partials->set_processor_structure(p, new contiguous_indexstruct(groupnumber) );
      ...
      REQUIRE_NOTHROW( partially_reduced = shared_ptr<distribution>( new product_distribution(partials) ) );
    }
    auto partial_sums;
    REQUIRE_NOTHROW( partial_sums = shared_ptr<object>( new product_object(partially_reduced) ) );
    ...
    SECTION( "group and sum separate" ) {
      ...
      REQUIRE_NOTHROW( local_groups = shared_ptr<object>( new product_object(locally_grouped) ) );
    ...
    REQUIRE_NOTHROW( top_summing->set_explicit_beta_distribution
		     ( shared_ptr<distribution>( new product_distribution(top_beta) ) ) );
...
TEST_CASE( "explore the reduction kernel","[reduction][kernel][56]" ) {
  auto
    local_scalar = shared_ptr<distribution>( new product_block_distribution(decomp,1,-1) ),
    global_scalar = shared_ptr<distribution>( new product_replicated_distribution(decomp) );
  ...
  auto
    local_value = shared_ptr<object>( new product_object(local_scalar) ),
    global_value = shared_ptr<object>( new product_object(global_scalar) );
...
TEST_CASE( "cg kernels","[cg][kernel][60]" ) {
  ...
  int nlocal = 1000;
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto blocked = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal,-1) );
  auto
    x = shared_ptr<object>( new product_object(blocked) ),
    xnew = shared_ptr<object>( new product_object(blocked) ),
    z = shared_ptr<object>( new product_object(blocked) ),
  ...
  SECTION( "scalar kernel error catching" ) {
    kernel *beta_calc;
    object *rr;
    REQUIRE_NOTHROW( rr = shared_ptr<object>( new product_object(blocked) ) );
  ...
  SECTION( "update" ) {
    double threeval = 3.;
    auto three = shared_ptr<object>( new product_object(scalar) );
    ...
    SECTION( "pp test s1" ) {
      three = shared_ptr<object>( new product_object(blocked) );
      REQUIRE_THROWS( pupdate = new product_axbyz_kernel
		      ( '+',three,z, '+',beta,pold, p ) );
    }
    SECTION( "pp test s2" ) {
      beta = shared_ptr<object>( new product_object(blocked) );
      REQUIRE_THROWS( pupdate = new product_axbyz_kernel
		      ( '+',three,z, '+',beta,pold, p ) );
    }
    SECTION( "pp" ) {
...
TEST_CASE( "neuron kernel","[kernel][sparse][DAG][70]" ) {
  ...
  auto blocked =
    shared_ptr<distribution>( new product_block_distribution(decomp,g) );
  auto
    a = shared_ptr<object>( new product_object(blocked) ),
    b = shared_ptr<object>( new product_object(blocked) ),
    c1 = shared_ptr<object>( new product_object(blocked) ), 
    c2 = shared_ptr<object>( new product_object(blocked) ),
    d = shared_ptr<object>( new product_object(blocked) );
