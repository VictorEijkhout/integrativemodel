...
TEST_CASE( "copy and operate distributions","[distribution][operate][copy][54]" ) {
  INFO( "mytid=" << mytid );
  int localsize = 10;
  auto d1 = shared_ptr<distribution>( new product_block_distribution(decomp,localsize,-1) );
  shared_ptr<distribution> d2;

  // SECTION( "plain copy" ) {
  //   REQUIRE_NOTHROW( d2 = shared_ptr<distribution>( new product_distribution( d1 ) ) );
  //   index_int s1,s2;
  ...
  SECTION( "operated copy" ) {
    ...
    REQUIRE_NOTHROW( ps = *( dynamic_cast<parallel_structure*>(d1.get()) ) );
    REQUIRE_NOTHROW( po = ps.operate(is) );

    // identical
    REQUIRE_NOTHROW( d2 = shared_ptr<distribution>( new product_distribution(ps ) ) );
    ...
    REQUIRE_NOTHROW( d2 = shared_ptr<distribution>( new product_distribution(po ) ) );
    CHECK( d2->volume(mycoord)==d1->volume(mycoord) );
...
TEST_CASE( "merge distributions","[indexstruct][merge][56]" ) {
  ...
  REQUIRE_NOTHROW( pstr = shared_ptr<distribution>( new product_block_distribution(decomp,-1,gsize) ) );
...
TEST_CASE( "divide distributions","[distribution][operate][57]" ) {
  ...
  auto twoper = shared_ptr<distribution>( new product_block_distribution(decomp,2*omp_nprocs,-1) );
...
//   CHECK_NOTHROW( d1 = shared_ptr<distribution>( new product_block_distribution
