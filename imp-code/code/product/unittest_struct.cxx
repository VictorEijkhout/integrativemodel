...
TEST_CASE( "Environment is proper","[environment][init][0]" ) {
...

  {
    ...
    CHECK_NOTHROW( d = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal,-1) ) );
    //INFO( format("product distribution with nlocal={}: {}",d->as_string()) );
    CHECK( d->global_volume()==nlocal*mpi_nprocs );
    CHECK( d->volume(mycoord)==nlocal );

    shared_ptr<object> v;
    REQUIRE_NOTHROW( v = shared_ptr<object>( new product_object(d) ) );
    int nt;
...
TEST_CASE( "test product structure","[embed][distribution][2]" ) {
  ...
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,nglobal) );
  ...
  auto xdata = data_allocate(nodelocal);
  {
    index_int
    ...
    for (int i=0; i<nodelocal; i++)
      xdata->at(i) = pointfunc33(i,my_first);
  }

  auto
    xvector = shared_ptr<object>( new product_object(block,xdata) ),
    yvector = shared_ptr<object>( new product_object(block) );
  ...
  auto scale = shared_ptr<kernel>( new product_kernel(xvector,yvector) );
  scale->add_sigma_operator( no_op );
  REQUIRE_NOTHROW( scale->set_last_dependency().set_name("33dep") );
  REQUIRE_NOTHROW( embedded_decomp = scale->get_out_object()->get_embedded_decomposition() );

  const char *path;
  int
    saved_omp_nprocs{omp_nprocs}, saved_mpi_nprocs{mpi_nprocs};
  index_int saved_global{nglobal};

  SECTION( "proc count" ) { path = "proc count";
    scale->set_name("testp");
    CHECK_NOTHROW
      ( scale->set_localexecutefn // this sets the omp-local function
	( [saved_mpi_nprocs] ( kernel_function_args ) -> void {
	  test_nprocs( kernel_function_call, saved_mpi_nprocs ); } ) );
  }
  // VLE function needs to be written 
  // SECTION( "distr proc count" ) {path = "distr proc count";
  //   scale->set_name("testp");
  //   CHECK_NOTHROW
  //     ( scale->set_localexecutefn
  // 	( [saved_omp_nprocs] ( kernel_function_args ) -> void {
  // 	  test_embedded_nprocs( kernel_function_call, saved_omp_nprocs ); } ) );
  // }
  SECTION( "global size" ) {path = "global size";
    scale->set_name("tests");
    CHECK_NOTHROW
      ( scale->set_localexecutefn
	( [saved_global] ( kernel_function_args ) -> void {
	  test_globalsize( kernel_function_call,saved_global ); } ) );
  }
  INFO( format("embedded decomposition: {}",embedded_decomp.as_string()) );
  INFO( "path: " << path );
  CHECK_NOTHROW( scale->analyze_dependencies(true) );
  CHECK_NOTHROW( scale->execute() );
}

TEST_CASE( "Analyze single interval","[index][structure][12]" ) {
  int localsize = 5;
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto dist = shared_ptr<distribution>( new product_distribution(decomp) ); 
  parallel_structure pstruct(decomp);
  CHECK_NOTHROW( pstruct.create_from_global_size(localsize*mpi_nprocs) );
  //  print("[12] early return!\n"); return;

  REQUIRE_NOTHROW( dist = shared_ptr<distribution>( new product_distribution(pstruct) ) );
  vector<shared_ptr<message>> mm;
  shared_ptr<message> m; shared_ptr<multi_indexstruct> s,segment;
  ...
  SECTION( "find a sub interval" ) {
    ...
    //    REQUIRE( s!=nullptr );
    CHECK( s->first_index_r().coord(0)==1 );
...
TEST_CASE( "Analyze one dependency","[operate][dependence][14]") {
  ...
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto alpha = 
    shared_ptr<distribution>( new product_block_distribution(decomp,localsize,-1) );
  ...
  //  for (int mytid=0; mytid<env->nprocs(); mytid++) {
  {
    ...
    auto my_size = (my_last-my_first+1);
    CHECK( my_size.volume()==localsize );
...
TEST_CASE( "Analyze one dependency modulo","[operate][dependence][modulo][16]") {
  ...
  int localsize = 100*omp_nprocs; int dim=1;
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto alpha = 
    shared_ptr<distribution>( new mpi_block_distribution(decomp,localsize,-1) );
  ioperator shiftop; shared_ptr<multi_indexstruct> alpha_block;
  vector<shared_ptr<message>> mm; shared_ptr<message> m;
  index_int my_first,my_last,
  ...
  //for (int mytid=0; mytid<env->get_architecture()->nprocs(); mytid++) { ????
  {
    ...
    SECTION( "right modulo" ) {
      ...
      CHECK( alpha_block->first_index_r()==my_first );
      CHECK( alpha_block->last_index_r()==my_last );
      auto alpha_size = (alpha_block->last_index_r()-alpha_block->first_index_r()+1);
      CHECK( alpha_size.volume()==localsize );
	...
	for (int imsg=0; imsg<2; imsg++) {
	  ...
	  if (m->get_sender().coord(0)==mytid) {
	    ...
	    CHECK( m->get_local_struct()->first_index_r()==domain_coordinate_allones(1) );
	  } else {
...
TEST_CASE( "Task dependencies threepoint bump","[task][message][object][21]" ) {
  ...
  auto d1 = 
    shared_ptr<distribution>( new product_block_distribution(decomp,gsize) );

  shared_ptr<object> o1,r1;
  CHECK_NOTHROW( r1 = shared_ptr<object>( new product_object(d1) ) );
  CHECK( r1->global_volume()==gsize );
  CHECK_NOTHROW( r1->set_name("21result") );
  CHECK_NOTHROW( o1 = shared_ptr<object>( new product_object(d1) ) );
  ...
  REQUIRE_NOTHROW( beta_struct.set_structure_type( beta_struct.infer_distribution_type() ) );
  ...
  CHECK_NOTHROW( beta_dist = shared_ptr<distribution>( new product_distribution(beta_struct) ) );
  ...
  SECTION( "make kernel" ) {
    ...
    REQUIRE_NOTHROW( combine = shared_ptr<kernel>( new product_kernel(o1,r1) ) );
    combine->set_name("combine-kernel-21");
    REQUIRE_NOTHROW( combine->set_explicit_beta_distribution( beta_dist ) );
    CHECK_NOTHROW( combine->set_last_dependency().set_name("22xbeta") );
  ...
  //   CHECK_NOTHROW( combine_task->set_last_dependency().set_name("22xbeta") );
  //   //CHECK_NOTHROW( combine_task->set_beta_distribution( 0,beta_dist ) );
  //   REQUIRE_NOTHROW( combine_task->analyze_dependencies() );
  // }

  // VLE Why? we have already done the analysis.
  // REQUIRE_NOTHROW( combine_task->endow_beta_objects(r1) );
...
TEST_CASE( "Task send structure threepoint modulo","[task][message][object][modulo][24]" ) {
  ...
  auto d1 = 
    shared_ptr<distribution>( new product_block_distribution(decomp,10*omp_nprocs*mpi_nprocs) );
  ...
  shared_ptr<task> combine_task;
  auto combine = shared_ptr<kernel>( new product_kernel(o1,r1) );
  ...
  //   CHECK_NOTHROW( combine_task->endow_beta_object() );
  // REQUIRE_NOTHROW( combine_task->set_last_dependency().endow_beta_object(r1) );
  // REQUIRE_NOTHROW( combine_task->derive_receive_messages(/*0,mytid*/) );
...
TEST_CASE( "Halo object for task modulo","[task][halo][modulo][25]" ) {
  ...
  auto d1 = 
    shared_ptr<distribution>( new product_block_distribution(decomp,nodesize*mpi_nprocs) );
  shared_ptr<object> o1,r1;
  const char *r1name = "result_vector_1",
    *o1name = "origin_vector_1";
  CHECK_NOTHROW( r1 = shared_ptr<object>( new product_object(d1) ) ); // result vector
  r1->set_name( r1name );
  CHECK( r1->get_name().compare(r1name)==0 );

  CHECK_NOTHROW( o1 = shared_ptr<object>( new product_object(d1) ) ); // origin vector
  o1->set_name( o1name );

  //  parallel_indexstruct *beta_struct;
  shared_ptr<object> halo;
  shared_ptr<kernel> combine; shared_ptr<task> combine_task;
  CHECK_NOTHROW( combine = shared_ptr<kernel>( new product_kernel(o1,r1) ) );
  signature_function *beta = new signature_function();

  SECTION( "no-op" ) {
    REQUIRE_NOTHROW( combine->set_last_dependency().add_sigma_operator( no_op ) );
    REQUIRE_NOTHROW( combine->analyze_dependencies() );
    REQUIRE_NOTHROW( combine_task = combine->get_tasks().at(0) );
  ...
  SECTION( "no-op and left" ) {
    CHECK_NOTHROW( combine->set_last_dependency().add_sigma_operator( no_op ) );
    CHECK_NOTHROW( combine->set_last_dependency().add_sigma_operator( left_shift ) );
    REQUIRE_NOTHROW( combine->analyze_dependencies() );
    REQUIRE_NOTHROW( combine_task = combine->get_tasks().at(0) );
    REQUIRE_NOTHROW( halo = combine_task->get_beta_object(0) );
  ...
  SECTION( "no-op and right" ) {
    CHECK_NOTHROW( combine->set_last_dependency().add_sigma_operator( no_op ) );
    CHECK_NOTHROW( combine->set_last_dependency().add_sigma_operator( right_shift ) );
    REQUIRE_NOTHROW( combine->analyze_dependencies() );
    REQUIRE_NOTHROW( combine_task = combine->get_tasks().at(0) );
    REQUIRE_NOTHROW( halo = combine_task->get_beta_object(0) );
...
TEST_CASE( "Halo object for kernel modulo","[kernel][halo][modulo][27]" ) {
  ...
  SECTION( "no-op" ) {
    ...
    REQUIRE_NOTHROW( combine->set_last_dependency().endow_beta_object(r1) );
...
TEST_CASE( "Halo object for kernel bump","[kernel][halo][28]" ) {
  ...
  auto
    ...
    left_shift  = ioperator("<=1");
  auto d1 = 
    shared_ptr<distribution>( new product_block_distribution(decomp,10*mpi_nprocs) );
  shared_ptr<object> o1,r1;
  CHECK_NOTHROW( r1 = shared_ptr<object>( new product_object(d1) ) ); // result vector
  CHECK_NOTHROW( o1 = shared_ptr<object>( new product_object(d1) ) ); // origin vector
  ...
  SECTION( "no-op" ) {
    ...
    CHECK_NOTHROW( combine->split_to_tasks() );
    CHECK( combine->get_tasks().size()==1 );
    //    CHECK_NOTHROW( combine->set_last_dependency().endow_beta_object(r1) );
    CHECK_NOTHROW( tsk = combine->get_tasks().at(0) );
    CHECK( tsk->get_beta_object(0)!=nullptr );
...
TEST_CASE( "Origin task execute on local data","[mpi][object][task][kernel][execute][29]" ) {
  ...
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,s*mpi_nprocs) );
  CHECK( block->volume(mycoord)==s );
  auto vector = shared_ptr<object>( new product_object(block) );
  decltype( vector->get_data(mycoord) ) data;
  REQUIRE_NOTHROW( data = vector->get_data(mycoord) );
  INFO( "object has data @ " << (long)(data.data()) );
  vector->set_name("product29 vector");
  
  // define a task
  shared_ptr<kernel> k;
  REQUIRE_NOTHROW( k = shared_ptr<kernel>( new product_origin_kernel(vector) ) );
  ...
  INFO( "task out object has data @ " << (long)(data.data()) );
  ...
  product_task *pd;
  REQUIRE_NOTHROW( pd = dynamic_cast<product_task*>(t.get()) );
  REQUIRE( pd!=nullptr );
  REQUIRE_NOTHROW( tsks = pd->get_node_tasks() );
  //  REQUIRE_NOTHROW( tsks = t->get_node_tasks() );
  //  REQUIRE( tsks!=nullptr );
  ...
  REQUIRE_NOTHROW( tsks = ( dynamic_cast<product_task*>(t.get()) )->get_node_tasks() );
  REQUIRE( tsks.size()==omp_nprocs );
  print("Early return from [29]\n"); return;
  ...
  REQUIRE_NOTHROW( data = vector->get_data(mycoord) );
  INFO( "after exec: object has data @ " << (long)(data.data()) );
  for (int p=0; p<omp_nprocs; p++) {
    for (int i=0; i<ss; i++) {
      INFO( "thread=" << p << ", i: " << i );
      CHECK( data.at(p*ss+i)==p+.5 );
    }
  }
}
...
TEST_CASE( "Task execute on external data","[mpi][object][task][kernel][execute][30]" ) {
  ...
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,s*mpi_nprocs) );
  auto xvector = shared_ptr<object>( new product_object(block,xdata) );

  // define a task
  auto xk = shared_ptr<kernel>( new product_kernel(xvector) );
  auto xt = shared_ptr<task>( new product_task(mycoord,xk.get()) );
  ...
  decltype( xvector->get_data(mycoord) ) data;
...
TEST_CASE( "Shift left bump kernel, execute","[task][kernel][halo][execute][32]" ) {
  ...
  int nlocal=10;
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal*mpi_nprocs) );
  ...
  for (int i=0; i<nlocal; i++)
    xdata->at(i) = pointfunc33(i,my_first);
  auto shift = shared_ptr<kernel>( new product_kernel(xvector,yvector) );
  ...
  decltype( halo->get_data(mycoord) ) halo_data;
  ...
  for ( auto t : tsks ) {
    ...
    shared_ptr<object> omp_in,omp_out;
    //double *data;
    shared_ptr<distribution> halodist;
    index_int thr_first, thr_last, offset; 
    REQUIRE_NOTHROW( omp_out = t->get_out_object() );
    INFO( "output object: " << omp_out->get_name() );
    decltype( omp_out->get_data(mycoord) ) outdata;
	...
	CHECK( omp_in->global_volume()==nlocal+1 );
      decltype( omp_in->get_data(mycoord) ) indata;
	...
	offset = omp_out->numa_offset();
	//get_numa_structure()->first_index_r()[0] - omp_out->get_global_structure()->first_index_r()[0];
	for (index_int i=thr_first; i<=thr_last; i++) {
  ...
  decltype( yvector->get_data(mycoord) ) ydata;
  CHECK_NOTHROW( ydata = yvector->get_data(mycoord) );
  {
    ...
    for (int i=0; i<nlocal; i++) {
      INFO( "i=" << i << " yi=" << ydata[i] );
      CHECK( ydata.at(i) == Approx(pointfunc33(i+1,my_first)) );
    }
  }
}
...
TEST_CASE( "Scale kernel","[task][kernel][execute][33]" ) {
    ...
    
  {
	...
	for (int p=0; p<omp_nprocs; p++) {
	  ...
	  REQUIRE_NOTHROW( pcoord = ompdecomp.coordinate_from_linear(p) );
	  INFO( "thread " << p );
	  decltype( out_object->get_data(pcoord) ) data;
	...
	shared_ptr<object> in_object,out_object;
	...
	for (int p=0; p<omp_nprocs; p++) {
	  ...
	  INFO( "thread " << p );
	  decltype( in_object->get_data(pcoord) ) data;
...
TEST_CASE( "Shift kernel modulo","[task][kernel][halo][modulo][execute][34]" ) {
  ...
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,nodelocal*mpi_nprocs) );
  auto 
    xvector = shared_ptr<object>( new product_object(block) ),
    yvector = shared_ptr<object>( new product_object(block) );
  index_int
    my_first = block->first_index_r(mycoord).coord(0),
    my_last = block->last_index_r(mycoord).coord(0);
  CHECK( block->volume(mycoord)==nodelocal );
  {
    ...
    REQUIRE_NOTHROW( data = xvector->get_data(mycoord) );
    for (int i=0; i<nodelocal; i++)
      data.at(i) = pointfunc33(i,my_first);
    REQUIRE_NOTHROW( data = yvector->get_data(mycoord) );
    for (int i=0; i<nodelocal; i++)
      data.at(i) = -37; // put a recognizable value
  }
  auto shift = shared_ptr<kernel>( new product_kernel(xvector,yvector) );
  ...
  SECTION( "by task" ) {
    ...
    vector<shared_ptr<message>> msgs; int mcount;
...

  {
	...
	decltype( in_object->get_data(mycoord) ) data;
	...
	// analyze in object
	{
	  ...
	  decltype( in_object->get_data_p(p) ) data;
	...
	// same with out
	{
	  ...
	  decltype( out_object->get_data_p(p) ) data;
...
TEST_CASE( "Shift from left kernel, message structure","[task][kernel][halo][35]" ) {
  ...
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,nodelocal,-1) );
  auto xdata = data_allocate(nodelocal);
  auto xvector = shared_ptr<object>( new product_object(block,xdata) ),
    yvector = shared_ptr<object>( new product_object(block) );
  index_int
...
TEST_CASE( "Shift right kernel, execute","[task][kernel][halo][execute][36]" ) {
  ...
  CHECK_NOTHROW( halo_data = halo->get_data(mycoord) );
  {
	...
	CHECK( halo_data.at(i) == Approx(pointfunc33(i,my_first)) );
      ...
      for (i=0; i<nlocal; i++) {
	INFO( "i=" << i << " hi=" << halo_data.at(i) );
	CHECK( halo_data.at(i) == Approx(pointfunc33(i-1,my_first)) );
      }
    }
  }
  ...
  for ( auto t : tsks ) {
    auto thr = t->get_domain();
    INFO( "process " << mytid << ", thread " << thr.as_string() );	  
    shared_ptr<object> omp_in,omp_out; double *data; shared_ptr<distribution> halodist;
    index_int thr_first, thr_last, offset; 
    REQUIRE_NOTHROW( omp_out = t->get_out_object() );
    INFO( "output object: " << omp_out->get_name() );
    decltype( omp_out->get_data(mycoord) ) outdata;
...
TEST_CASE( "Shift from left kernel modulo, execute","[task][kernel][halo][execute][modulo][37]" ) {
  ...
  CHECK_NOTHROW( shift->execute() );
  decltype( shift_task->get_beta_object(0)->get_data(mycoord) ) halo_data;
...
TEST_CASE( "Scale queue","[queue][execute][100]" ) {
  ...
  auto no_op =  ioperator("none") ;
  auto block = 
    shared_ptr<distribution>( new product_block_distribution(decomp,nlocal*mpi_nprocs) );
  index_int
    my_first = block->first_index_r(mycoord).coord(0), my_last = block->last_index_r(mycoord).coord(0);
  ...
  for (int iv=0; iv<nsteps; iv++) {
    yvector[iv] = shared_ptr<object>( new product_object(block) );
  }

  shared_ptr<algorithm> queue;
  CHECK_NOTHROW( queue = shared_ptr<algorithm>( new product_algorithm(decomp) ) );
  queue->set_name("scale queue");
  {
    auto k = shared_ptr<kernel>( new product_kernel(xvector) );
    k->set_name( "generate" );
  ...
  for (int iv=0; iv<nsteps; iv++) {
    ...
    if (iv==0) {
      k = shared_ptr<kernel>( new product_kernel(xvector,yvector[0]) );
    } else {
      k = shared_ptr<kernel>( new product_kernel(yvector[iv-1],yvector[iv]) );
    }
  ...
  { // analyze the embedded queue
    for ( auto mpitsk : tsks ) {
      ...
      shared_ptr<algorithm> node_queue;
  ...
  {
    int i;
    decltype( yvector[nsteps-1]->get_data(mycoord) ) ydata;
...
TEST_CASE( "Threepoint queue mod","[queue][execute][modulo][halo][101]" ) {
  ...
  REQUIRE_NOTHROW( block = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal*mpi_nprocs) ) );
  index_int
    my_first = block->first_index_r(mycoord).coord(0), my_last = block->last_index_r(mycoord).coord(0);
  CHECK( my_first==mytid*nlocal );
  CHECK( my_last==(mytid+1)*nlocal-1 );
  ...
  for (int i=0; i<nlocal; i++)
    xdata->at(i) = 1.;
  for (int iv=0; iv<nsteps; iv++) {
    REQUIRE_NOTHROW( yvector[iv] = shared_ptr<object>( new product_object(block) ) );
  }
  ...
  {
    shared_ptr<kernel> k;
    REQUIRE_NOTHROW( k = shared_ptr<kernel>( new product_kernel(xvector) ) );
    k->set_localexecutefn( &vecnoset );
    CHECK_NOTHROW( queue->add_kernel(k) );
  }
  for (int iv=0; iv<nsteps; iv++) {
    shared_ptr<kernel> k;
    if (iv==0) {
      REQUIRE_NOTHROW( k = shared_ptr<kernel>( new product_kernel(xvector,yvector[0]) ) );
    } else {
      REQUIRE_NOTHROW( k = shared_ptr<kernel>( new product_kernel(yvector[iv-1],yvector[iv]) ) );
    }
    k->add_sigma_operator( no_op );
  ...
  vector<shared_ptr<task>> predecessors,tsks;
  REQUIRE_NOTHROW( tsks = queue->get_tasks() );
  for ( auto t : tsks ) {
  ...
  return;
  {
    for (int s=0; s<nsteps; s++) {
      int i;
      decltype( yvector[s]->get_data(mycoord) ) ydata;
...
TEST_CASE( "Shift from left kernel bump, execute","[task][kernel][halo][execute][modulo][38]" ) {
  ...
  int nlocal=10;
  auto block = 
    shared_ptr<distribution>( new product_block_distribution(decomp,-1,nlocal*mpi_nprocs) );
  int nglobal = block->global_volume();
  auto xdata = data_allocate(nlocal);
  auto xvector = shared_ptr<object>( new product_object(block,xdata) );
  auto yvector = vector<shared_ptr<object>>(nsteps);
...
TEST_CASE( "Add with left right modulo, execute","[task][kernel][halo][execute][modulo][39]" ) {
  ...
  for (int i=0; i<nlocal; i++)
    xdata->at(i) = pointfunc33(i,my_first);
  auto sum = shared_ptr<kernel>( new product_kernel(xvector,yvector) );
  sum->set_name("39sum");
  ...
  decltype( sum_task->get_beta_object(0)->get_data(mycoord) ) halo_data;
...
TEST_CASE( "Test explicit beta","[beta][distribution][40]" ) {
  ...
  auto in = shared_ptr<object>( new product_object(block) ),
    out = shared_ptr<object>( new product_object(block) );
  shared_ptr<kernel> threepoint;
  REQUIRE_NOTHROW( threepoint = shared_ptr<kernel>( new product_kernel(in,out) ) );
  ...
  auto indata = in->get_data(mycoord);
  CHECK( in->volume(mycoord)==localsize );
  for (index_int i=0; i<localsize; i++)
    indata[i] = 2.;
  REQUIRE_NOTHROW( threepoint->auto double *outdata = out->get_data(mycoord);
...
TEST_CASE( "Beta from sparse matrix","[beta][sparse][41]" ) {
  ...
  auto last_coordinate = domain_coordinate( vector<index_int>{localsize-1} );
  auto block = shared_ptr<distribution>( new product_block_distribution(decomp,localsize,-1) );
  auto in_obj = shared_ptr<object>( new product_object(block) ),
    out_obj = shared_ptr<object>( new product_object(block) );
  {
    auto indata = in_obj->get_data(mycoord); index_int n = in_obj->volume(mycoord);
    for (index_int i=0; i<n; i++) indata[i] = .5;
  }
  auto kern = shared_ptr<kernel>( new product_kernel(in_obj,out_obj) );
  ...
  SECTION( "connect right" ) {
    ...
    for ( m : rmsgs ) { // (vector<shared_ptr<message>>::iterator m=rmsgs->begin(); m!=rmsgs->end(); ++m) {
...
TEST_CASE( "Actual sparse matrix","[beta][sparse][42]" ) {
  ...
  auto spmvp = shared_ptr<kernel>( new product_kernel(in_obj,out_obj) );
...

  {
    ...
    shared_ptr<distribution> structure;
...
TEST_CASE( "Sparse matrix kernel","[beta][sparse][43]" ) {
  ...
  auto spmvp = shared_ptr<kernel>( new product_spmvp_kernel(in_obj,out_obj,mat) );
  REQUIRE_NOTHROW( spmvp->analyze_dependencies() );
...
TEST_CASE( "Kernel data graph: test acyclicity","[kernel][50]" ) {
  ...
  SECTION( "unnamed objects" ) {
    object1 = shared_ptr<object>( new product_object(block) );
    object2 = shared_ptr<object>( new product_object(block) );
    object3 = shared_ptr<object>( new product_object(block) );
    object4 = shared_ptr<object>( new product_object(block) );

    kernel1 = shared_ptr<kernel>( new product_kernel(object1,object2) );
    kernel1->set_name("50kernel1");
    kernel2 = shared_ptr<kernel>( new product_kernel(object1,object3) );
    kernel2->set_name("50kernel2");
    kernel3 = shared_ptr<kernel>( new product_kernel(object2,object3) );
    kernel3->set_name("50kernel3");

    queue = shared_ptr<algorithm>( new product_algorithm(decomp) );
  ...
  SECTION( "named objects" ) {
    ...
    vector<kernel<product_message>*> *use;
    // there are two kernels that use object1, none that generate it
  ...
  SECTION( "kernel predecessors" ) {
    ...
    kernel1 = shared_ptr<kernel>( new product_kernel(object1,object2) );
    kernel1->set_name("make2");
    kernel2 = shared_ptr<kernel>( new product_kernel(object2,object3) );
    kernel2->set_name("make3");
...
TEST_CASE( "Mapping between redundant distros","[redundant][70]" ) {
  ...
  auto 
    din = shared_ptr<distribution>( new product_replicated_scalar(env) ),
    dout = shared_ptr<distribution>( new product_replicated_scalar(env) );
  auto scalar_in = shared_ptr<object>( new product_object( din ) ),
    scalar_out = shared_ptr<object>( new product_object( dout ) );
  auto
    indata = scalar_in->get_data(mycoord),
    outdata = scalar_out->get_data(mycoord);
  *indata = 15.3;

  auto copy_kernel = shared_ptr<kernel>( new product_kernel( scalar_in,scalar_out ) );
  copy_kernel->set_explicit_beta_distribution
    ( dout->get_environment(),dout->get_processor_structure() );
  shared_ptr<task> copy_task;

  REQUIRE_NOTHROW( copy_kernel->set_last_dependency().endow_beta_object(scalar_out ) );
...
TEST_CASE( "Interpolation by restriction", "[stretch][distribution][71]" ) {
  int nlocal=10;
  auto
    target_dist = shared_ptr<distribution>( new product_block_distribution(decomp,nlocal*mpi_nprocs) ),
    source_dist = shared_ptr<distribution>( new product_block_distribution(decomp,2*nlocal*mpi_nprocs) );
  index_int
    myfirst = target_dist->first_index_r(mycoord).coord(0), mylast = target_dist->last_index_r(mycoord).coord(0);
  auto target = shared_ptr<object>( new product_object(target_dist) ),
    source = shared_ptr<object>( new product_object(source_dist) );
  auto restrict = shared_ptr<kernel>( new product_kernel(source,target) );
...

  {
    ...
    SECTION( "carefully" ) {
      msgs = source_dist->messages_for_segment
...
TEST_CASE( "Multistage tree collecting, explicit distributions","[redundant][72][hide]" ) {
  ...
  shared_ptr<distribution> twoper,unique,redund;   // alpha/gamma distros
  shared_ptr<distribution> duplic; shared_ptr<distribution> twosies; // beta distros
  shared_ptr<object> bot,mid,top;
  shared_ptr<kernel> gather1,gather2;
  auto div2 =  ioperator("/2") ;
  auto ul2 =  ioperator("x2") ;

  INFO( "mytid=" << mytid );

  twoper = shared_ptr<distribution>( new product_block_distribution(decomp,2,-1) );
  bot = shared_ptr<object>( new product_object(twoper) );
  ...
  SECTION( "derive the unique distro" ) {
    SECTION( "twoper->unique, explicit" ) {
      unique = shared_ptr<distribution>( new product_block_distribution(decomp,1,-1) );
    }
    SECTION( "twoper->unique, recursive" ) {
      REQUIRE_NOTHROW( unique = (product_distribution*)twoper->operate( div2 ) );
    }
    mid = shared_ptr<object>( new product_object(unique) );
  ...
  SECTION( "take it from the unique definition" ) {
    ...
    mid = shared_ptr<object>( new product_object(unique) );
    gather1 = shared_ptr<kernel>( new product_kernel(bot,mid) );

    SECTION( "create the redundant distro" ) {
      SECTION( "redund, explicit" ) {
	redund = shared_ptr<distribution>( new product_distribution(decomp,&hlf,1) );
      }
	...
	SECTION( "it's really the bottom alpha" ) {
	  //REQUIRE_NOTHROW( twosies = shared_ptr<distribution>( new product_distribution( *twoper ) ) );
	  twosies = twoper;
	}
	SECTION( "but we can also do it from mid" ) {
	  shared_ptr<distribution> twicegamma,shiftgamma;
	...
	twosies = shared_ptr<distribution>( new product_distribution( *twoper ) );
	...
	top = shared_ptr<object>( new product_object(redund) );
	...
	gather2 = shared_ptr<kernel>( new product_kernel(mid,top) );
	...
	SECTION( "duplicate, explicit" ) {
	  duplic = shared_ptr<distribution>( new product_distribution(decomp,&dup,2) ); // this is the tricky one
	}
...
void scansum(int step,int p,shared_ptr<object> invector,shared_ptr<object> outvector,void *ctx)
{
  auto indata = invector->get_data(mycoord);
  int insize = invector->volume(p);

  auto outdata = outvector->get_data(mycoord);
...
TEST_CASE( "multistage tree collecting, recursive","[distribution][redundant][73]" ) {
  ...
  auto twoper = shared_ptr<distribution>( new product_block_distribution(decomp,2,-1) );
  auto bot = shared_ptr<object>( new product_object(twoper) );
  {
    CHECK( bot->volume(mycoord)==2 );
    auto botdata = bot->get_data(mycoord);
    botdata[0] = botdata[1] = 1.;
  }
  ...
  REQUIRE_NOTHROW( unique = (product_distribution*)twoper->operate( div2 ) );
  REQUIRE_NOTHROW( mid = shared_ptr<object>( new product_object(unique) ) );
  CHECK( mid->volume(mycoord)==1 );
  CHECK( mid->first_index_r(mycoord).coord(0)==mytid );

  REQUIRE_NOTHROW( redund = (product_distribution*)unique->operate( div2 ) );
  REQUIRE_NOTHROW( top = shared_ptr<object>( new product_object(redund) ) );
  CHECK( top->volume(mycoord)==1 );
  CHECK( top->first_index_r(mycoord).coord(0)==(mytid/2) );
  CHECK( top->last_index_r(mycoord).coord(0)==top->first_index_r(mycoord).coord(0) );
  ...
  REQUIRE_NOTHROW( gather1 = shared_ptr<kernel>( new product_kernel(bot,mid) ) );
  ...
  {
    auto data = mid->get_data(mycoord); 
    CHECK( data[0]==2. );
  }


  shared_ptr<kernel> gather2;
  REQUIRE_NOTHROW( gather2 = shared_ptr<kernel>( new product_kernel(mid,top) ) );
  ...
  // shared_ptr<distribution> duplic,twicegamma,shiftgamma;
  // REQUIRE_NOTHROW( twicegamma = (product_distribution*)redund->operate(mul2) );
  // CHECK( twicegamma->first_index_r(mycoord).coord(0)==2*redund->first_index_r(mycoord).coord(0) );
  // CHECK( twicegamma->volume(mycoord)==redund->volume(mycoord) );
  // index_int s = redund->volume(mycoord);
...
TEST_CASE( "multistage tree collecting iterated","[distribution][redundant][74]" ) {
  ...
  auto div2 =  ioperator("/2") ;
  auto mul2 =  ioperator("x2") ;
  vector<shared_ptr<distribution>> distributions(nlevels);
  shared_ptr<object> objects[nlevels];
  for (int nlevel=0; nlevel<nlevels; nlevel++) {
    if (nlevel==0) {
      distributions.at(0)
	= shared_ptr<distribution>( new product_block_distribution(decomp,points_per_proc,-1) );
    } else {
      distributions.at(nlevel) = distributions[nlevel-1]->operate(div2);
    }
    INFO( "level: " << nlevel << "; g=" << distributions[nlevel]->global_volume() );
    objects[nlevel] = shared_ptr<object>( new product_object(distributions[nlevel]) );
    //snippet end
    if (nlevel>0) {
  ...
  for (int nlevel=0; nlevel<nlevels-1; nlevel++) {
    ...
    kernels[nlevel] =
      shared_ptr<kernel>( new product_kernel(objects[nlevel],objects[nlevel+1]) );
  ...
  // does this work?
  decltype( objects[0]->get_data(mycoord) ) data;
  int n;
...
TEST_CASE( "Scale queue with gen kernel","[queue][execute][103]" ) {
  ...
  double *xdata,*ydata;
  auto xvector = shared_ptr<object>( new product_object(block) ),
    yvector = shared_ptr<object>( new product_object(block) );
  auto gen_kernel = shared_ptr<kernel>( new product_kernel(xvector) ),
    mult_kernel = shared_ptr<kernel>( new product_kernel(xvector,yvector) );
