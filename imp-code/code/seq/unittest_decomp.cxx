...
TEST_CASE( "decomposition constructors" ) {
  {
    REQUIRE_NOTHROW( seq_decomposition<1>( the_env ) );
    seq_decomposition<1> d( the_env );
    REQUIRE( d.local_volume()==the_env.nprocs() );
    REQUIRE( d.global_volume()==the_env.nprocs() );
  }
  {
    REQUIRE_NOTHROW( seq_decomposition<2>( the_env ) );
    seq_decomposition<1> d( the_env );
    REQUIRE( d.local_volume()==the_env.nprocs() );
    REQUIRE( d.global_volume()==the_env.nprocs() );
  }
...
TEST_CASE( "decompositions","[seq][decomposition][01]" ) {
  auto ntids = the_env.nprocs();
  SECTION( "1D" ) {
    const int d=1;
    INFO( "1D" );
    REQUIRE_NOTHROW( seq_decomposition<d>(the_env) );
    seq_decomposition<d> decomp(the_env);
    int count = 0;
    for ( auto dom : decomp ) {
